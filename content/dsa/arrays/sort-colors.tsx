...
export default function SortColorsLab() {
  const [array, setArray] = useState<number[]>([2, 0, 2, 1, 1, 0]);
  ...
  const steps = useMemo<Step[]>(() => {
    ...
    s.push({
      type: "init", message: "Dutch National Flag Algorithm: Sorting 0s, 1s, and 2s.",
      line: 0, array: [...arr], low, mid, high,
      panelData: { low, mid, high, status: "Init" }
    });

    while (mid <= high) {
      s.push({
        type: "check", message: `Checking mid pointer at ${mid} (Value: ${arr[mid]}).`,
        line: 1, array: [...arr], low, mid, high,
        panelData: { low, mid, high, current: arr[mid] }
      });

      if (arr[mid] === 0) {
        [arr[low], arr[mid]] = [arr[mid], arr[low]];
        s.push({
          type: "swap0", message: `Found 0. Swap arr[low] and arr[mid]. Increment both.`,
          line: 2, array: [...arr], low, mid, high,
          panelData: { low, mid, action: "Swap(0)" }
        });
        low++;
        mid++;
      } else if (arr[mid] === 1) {
        s.push({
          type: "skip1", message: `Found 1. Correct position. Increment mid.`,
          line: 3, array: [...arr], low, mid, high,
          panelData: { mid, action: "Mid++" }
        });
        mid++;
      } else {
        [arr[mid], arr[high]] = [arr[high], arr[mid]];
        s.push({
          type: "swap2", message: `Found 2. Swap arr[mid] and arr[high]. Decrement high.`,
          line: 4, array: [...arr], low, mid, high,
          panelData: { mid, high, action: "Swap(2)" }
        });
        high--;
      }
    }
    ...
    s.push({
      type: "done", message: "Array sorted into [Red, White, Blue] categories.",
      line: null, array: [...arr], low, mid, high,
  ...
  return (
    <div style={{ width: "100vw", height:"calc(100vh - 124px)", overflowY: "auto", background: COLORS.bg, scrollbarWidth: "none" }}>
      <TheorySection 
        title="Sort Colors"
        definition="Also known as the Dutch National Flag problem. Sorting an array of three distinct values (e.g., 0s, 1s, and 2s) in linear time using constant space."
        timeComplexity="O(N)"
        spaceComplexity="O(1)"
        keyPoints={[
      ...
      <div style={{ height: 48, background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, display: "flex", alignItems: "center", padding: "0 16px", gap: 12 }}>
        <div style={{ fontSize: 14, fontWeight: 800, marginRight: 20 }}>SORT_COLORS_LAB</div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: COLORS.bg, padding: "4px 12px", borderRadius: 6, border: `1px solid ${COLORS.border}` }}>
          <span style={{ fontSize: 10, color: COLORS.textMuted }}>ARRAY (0,1,2):</span>
          <input 
            type="text" 
            placeholder="2, 0, 1..." 
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                ...
                const newArr = val.split(",").map(x => parseInt(x.trim())).filter(x => x === 0 || x === 1 || x === 2);
                if (newArr.length > 0) {
                  setArray(newArr);
                  setStepIdx(0);
                  setIsPlaying(false);
      ...
      <div style={{ flex: 1, display: "flex" }}>
        ...
        <div style={{ flex: 1, position: "relative" }}>
          ...
          {renderPanel("stats", "Pointer Tracking", (
          ...
          {renderPanel("logic", "DNF Algorithm", (
          ...
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <div style={{ display: "flex", gap: 12 }}>
              {step.array.map((val, i) => {
                const color = val === 0 ? COLORS.red : val === 1 ? COLORS.textWhite : COLORS.blue;
                ...
                return (
                  <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                    <div style={{
                      width: 50, height: 50, 
                      background: `rgba(${val === 0 ? '248,81,73' : val === 1 ? '201,209,217' : '88,166,255'}, 0.2)`,
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 2, alignItems: 'center' }}>
                      {isLow && <div style={{ fontSize: 9, color: COLORS.red, fontWeight: 800 }}>LOW</div>}
                      {isMid && <div style={{ fontSize: 9, color: COLORS.textWhite, fontWeight: 800 }}>MID</div>}
                      {isHigh && <div style={{ fontSize: 9, color: COLORS.blue, fontWeight: 800 }}>HIGH</div>}
