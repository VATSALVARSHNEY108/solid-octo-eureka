...
export default function ArrayTraversalLab() {
  ...
  const steps = useMemo<Step[]>(() => {
    const s: Step[] = [
      {
        type: "init",
        message: "Array traversal visiting each element exactly once.",
        line: 0,
        array: [...array],
        highlightIdx: null,
        panelData: { i: "undefined", element: "N/A" }
      }
    ];
    ...
    array.forEach((val, i) => {
      s.push({
        type: "visiting",
        message: `Visiting index ${i}: Value = ${val}`,
        line: 2,
        array: [...array],
        highlightIdx: i,
        panelData: { i, element: val, status: "Active" }
      });
    });

    s.push({
      type: "done",
      message: "End of array reached. Traversal O(n) finished.",
      line: 4,
      array: [...array],
      highlightIdx: null,
      panelData: { status: "Completed", complexity: "O(n)" }
    });
  ...
  return (
    <div style={{ width: "100vw", height:"calc(100vh - 124px)", overflowY: "auto", background: COLORS.bg, scrollbarWidth: "none" }}>
      <TheorySection 
        ...
        definition="The process of visiting each element in an array exactly once to perform some operation like printing, searching, or modifying data."
        timeComplexity="O(N)"
        spaceComplexity="O(1)"
        keyPoints={[
          "Linear time complexity",
          "Foundation for searching & sorting",
          "Iterates through indices 0 to N-1",
      ...
      <div style={{ height: 48, background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, display: "flex", alignItems: "center", padding: "0 16px", gap: 12 }}>
        <div style={{ fontSize: 14, fontWeight: 800, marginRight: 20 }}>TRAVERSAL_LAB</div>
        ...
        <button style={btnStyle} onClick={() => setArray([10,20,30,40,50])}>Static</button>
        <button style={btnStyle} onClick={() => setArray(Array.from({length:5}, () => Math.floor(Math.random()*99)))}>Random</button>
      </div>
      ...
      <div style={{ flex: 1, display: "flex" }}>
        ...
        <div style={{ flex: 1, position: "relative" }}>
          ...
          {renderPanel("stats", "Iteration Stats", (
          ...
          {renderPanel("logic", "Code Execution", (
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {array.map((val, i) => (
                <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                  <div style={{
                    width: 64, height: 64, background: step.highlightIdx === i ? "rgba(88,166,255,0.15)" : COLORS.surface,
                    border: `2px solid ${step.highlightIdx === i ? COLORS.blue : COLORS.border}`,
                    borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: 20, fontWeight: 700, color: step.highlightIdx === i ? COLORS.blue : COLORS.textWhite,
                    transition: "all 0.2s"
                  }}>{val}</div>
