...
export default function ArrayInsertionLab() {
  const [array, setArray] = useState<(number | null)[]>([10, 20, 30, 40, null]);
  ...
  const steps = useMemo<Step[]>(() => {
    return [
      {
        type: "init",
        message: "Inserting 25 at index 2. We must shift elements to make space.",
        line: 0,
        array: [...array],
        highlightIdx: 2,
        shiftIdx: null,
        panelData: { pos: 2, value: 25, size: array.filter(x => x !== null).length }
      },
      // ... (Rest of logic adapted to array size)
      {
        type: "done",
        message: "Insertion complete. Array updated.",
        line: 5,
        array: [...array],
        highlightIdx: null,
        shiftIdx: null,
        panelData: { size: array.length, complexity: "O(n)" }
      }
    ];
  }, [array]);
  ...
  return (
    <div style={{ width: "100vw", height:"calc(100vh - 124px)", overflowY: "auto", background: COLORS.bg, scrollbarWidth: "none" }}>
      <TheorySection 
        ...
        definition="Inserting an element into an array at a specific index. This often requires shifting subsequent elements to the right to maintain order."
        timeComplexity="O(N) (Worst Case)"
        spaceComplexity="O(1) (In-place)"
        keyPoints={[
          "Shifting required for internal indices",
      ...
      <div style={{ height: 48, background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, display: "flex", alignItems: "center", padding: "0 16px", gap: 12 }}>
        <div style={{ fontSize: 14, fontWeight: 800, marginRight: 20 }}>INSERTION_LAB</div>
          ...
          <input 
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                ...
                if (newArr.length > 0) {
                  setArray([...newArr, null]); // Extra space for insertion
                  setStepIdx(0);
                  setIsPlaying(false);
                }
              }
      ...
      <div style={{ flex: 1, display: "flex" }}>
        ...
        <div style={{ flex: 1, position: "relative" }}>
          ...
          {renderPanel("ops", "Insertion Data", (
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {step.array.map((val, i) => (
                <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                  <div style={{
                    width: 64, height: 64, 
                    background: step.highlightIdx === i ? "rgba(59,130,246,0.15)" : step.shiftIdx === i ? "rgba(240,136,62,0.15)" : COLORS.surface,
                    border: `2px solid ${step.highlightIdx === i ? COLORS.blue : step.shiftIdx === i ? COLORS.orange : COLORS.border}`,
                    borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: 20, fontWeight: 700, color: step.highlightIdx === i ? COLORS.blue : step.shiftIdx === i ? COLORS.orange : COLORS.textWhite,
                    transition: "all 0.2s"
                  }}>{val ?? "-"}</div>
