...
const CODE = [
  ...
  "int right = 1;",
  "for (int i = n-1; i >= 0; i--) { res[i] *= right; right *= arr[i]; }",
];

export default function ProductExceptSelfLab() {
  ...
  const steps = useMemo<Step[]>(() => {
    ...
    s.push({
      type: "init", message: "Goal: Product of all except self. O(n) time, no division.",
      line: 0, array, left: [], right: [], result: [...res], currentIdx: null,
    ...
    res[0] = 1;
    s.push({
      type: "leftInit", message: "Initialize res[0] = 1 (nothing to the left of index 0).",
      line: 0, array, left: [], right: [], result: [...res], currentIdx: 0,
      panelData: { i: 0, val: 1 }
    });
    ...
    for (let i = 1; i < n; i++) {
      ...
      s.push({
        type: "leftPass", message: `res[${i}] = res[${i-1}] * arr[${i-1}] = ${res[i]}`,
        line: 1, array, left: [], right: [], result: [...res], currentIdx: i,
        panelData: { i, leftProduct: res[i] }
      });
    }
    ...
    let right = 1;
    s.push({
      type: "rightInit", message: "Now traverse backward. Track cumulative product from the right.",
      line: 2, array, left: [], right: [], result: [...res], currentIdx: null,
      panelData: { rightProduct: 1 }
    });
    ...
    for (let i = n - 1; i >= 0; i--) {
      ...
      res[i] = res[i]! * right;
      s.push({
        type: "rightPass", message: `Index ${i}: res[${i}] (${oldRes}) * rightProduct (${right}) = ${res[i]}`,
        line: 3, array, left: [], right: [], result: [...res], currentIdx: i,
        panelData: { i, rightProduct: right, final: res[i] }
      });
      right *= array[i];
    }

    s.push({
      type: "done", message: "Final result array contains products of all elements except self.",
      line: null, array, left: [], right: [], result: [...res], currentIdx: null,
      panelData: { status: "Success", complexity: "O(n)" }
    });
  ...
  return (
    <div style={{ width: "100vw", height:"calc(100vh - 124px)", overflowY: "auto", background: COLORS.bg, scrollbarWidth: "none" }}>
      <TheorySection 
        ...
        definition="Calculating the product of all elements in an array except the one at the current index, without using the division operator."
        timeComplexity="O(N)"
      ...
      <div style={{ flex: 1, display: "flex" }}>
        ...
        <div style={{ flex: 1, position: "relative" }}>
          ...
          {renderPanel("stats", "Product Tracker", (
          ...
          {renderPanel("logic", "Two-Pass Algorithm", (
              ...
              {step.result.map((val, i) => (
                <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                  <div style={{
                    ...
                    borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: 18, fontWeight: 700, color: val === null ? COLORS.textDark : COLORS.green
                  }}>{val ?? "?"}</div>
                  <div style={{ fontSize: 9, color: COLORS.textDark }}>res[{i}]</div>
                </div>
              ))}
            </div>
          </div>
