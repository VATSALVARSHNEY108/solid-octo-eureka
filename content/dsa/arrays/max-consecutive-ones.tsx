...
export default function MaxConsecutiveOnesLab() {
  const [array, setArray] = useState<number[]>([1, 1, 0, 1, 1, 1, 0, 1]);
  ...
  const steps = useMemo<Step[]>(() => {
    ...
    s.push({
      type: "init", message: "Find the maximum number of consecutive 1s in the array.",
      line: 0, array, currentIdx: null, currentCount: 0, maxCount: 0,
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      s.push({
        type: "check", message: `Checking element at index ${i}: ${x}`,
        line: 1, array, currentIdx: i, currentCount, maxCount,
        panelData: { i, val: x, currentCount, maxCount }
      });

      if (x === 1) {
        currentCount++;
        s.push({
          type: "inc", message: "It's a 1! Increment current consecutive count.",
          line: 3, array, currentIdx: i, currentCount, maxCount,
          panelData: { i, val: x, currentCount, maxCount, action: "currentCount++" }
        });
        
        if (currentCount > maxCount) {
          maxCount = currentCount;
          s.push({
            type: "update_max", message: `New maximum consecutive count: ${maxCount}`,
            line: 4, array, currentIdx: i, currentCount, maxCount,
            panelData: { i, val: x, currentCount, maxCount, action: "maxCount = currentCount" }
          });
        }
      } else {
        currentCount = 0;
        s.push({
          type: "reset", message: "It's a 0! Reset current consecutive count to 0.",
          line: 6, array, currentIdx: i, currentCount, maxCount,
          panelData: { i, val: x, currentCount, maxCount, action: "currentCount = 0" }
        });
      }
    }

    s.push({
      type: "done", message: `Algorithm complete. Maximum consecutive 1s: ${maxCount}`,
      line: 9, array, currentIdx: null, currentCount, maxCount,
      panelData: { result: maxCount, status: "Done" }
    });
  ...
  return (
    <div style={{ width: "100vw", height:"calc(100vh - 124px)", overflowY: "auto", background: COLORS.bg, scrollbarWidth: "none" }}>
      <TheorySection 
        ...
        definition="Given a binary array, find the maximum number of consecutive 1s in this array."
        timeComplexity="O(N)"
        spaceComplexity="O(1)"
        keyPoints={[
          "Linear scan of the array",
          "Reset count on encountering 0",
      ...
      <div style={{ height: 48, background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, display: "flex", alignItems: "center", padding: "0 16px", gap: 12 }}>
        <div style={{ fontSize: 14, fontWeight: 800, marginRight: 20 }}>MAX_CONSEC_ONES_LAB</div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: COLORS.bg, padding: "4px 12px", borderRadius: 6, border: `1px solid ${COLORS.border}` }}>
          <span style={{ fontSize: 10, color: COLORS.textMuted }}>BINARY ARRAY:</span>
          <input 
            type="text" 
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                ...
                const newArr = val.split(",").map(x => parseInt(x.trim())).filter(x => x === 0 || x === 1);
                if (newArr.length > 0) {
                  setArray(newArr);
                  setStepIdx(0);
                  setIsPlaying(false);
      ...
      <div style={{ flex: 1, display: "flex" }}>
        ...
        <div style={{ flex: 1, position: "relative" }}>
          ...
          {renderPanel("stats", "Counter Stats", (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
                <span style={{ color: COLORS.textMuted }}>CURRENT COUNT</span>
                <span style={{ color: COLORS.blue, fontWeight: 800 }}>{step.currentCount}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
                <span style={{ color: COLORS.textMuted }}>MAX COUNT</span>
                <span style={{ color: COLORS.green, fontWeight: 800 }}>{step.maxCount}</span>
              </div>
            </div>
          ))}
              ...
              {array.map((val, i) => {
                ...
                return (
                  <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                    <div style={{
                      width: 50, height: 50, 
                      background: isCurrent ? "rgba(88,166,255,0.15)" : val === 1 ? "rgba(63,185,80,0.05)" : COLORS.surface,
                      border: `2px solid ${isCurrent ? COLORS.blue : val === 1 ? COLORS.green : COLORS.border}`,
                      borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center",
                      fontSize: 18, fontWeight: 700, color: val === 1 ? COLORS.green : COLORS.textDark,
                      transition: "all 0.3s"
                    }}>{val}</div>
