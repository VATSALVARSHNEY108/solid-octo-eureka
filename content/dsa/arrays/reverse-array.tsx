...
export default function ReverseArrayLab() {
  ...
  const steps = useMemo<Step[]>(() => {
    ...
    s.push({
      type: "init", message: "Initialize pointers at the boundaries.",
      line: 0, array: [...arr], left, right,
      panelData: { left, right, status: "Ready" }
    });

    while (left < right) {
      s.push({
        type: "swap", message: `Swapping index ${left} and ${right}.`,
        line: 2, array: [...arr], left, right,
        panelData: { left, right, action: "Swapping" }
      });
      
      [arr[left], arr[right]] = [arr[right], arr[left]];
      
      s.push({
        type: "move", message: "Move pointers inward.",
        line: 3, array: [...arr], left, right,
        panelData: { left, right, action: "Moving Pointers" }
      });
    ...
    s.push({
      type: "done", message: "Array successfully reversed in-place.",
      line: 5, array: [...arr], left: null, right: null,
      panelData: { status: "Completed", complexity: "O(n/2)" }
    });
  ...
  return (
    <div style={{ width: "100vw", height:"calc(100vh - 124px)", overflowY: "auto", background: COLORS.bg, scrollbarWidth: "none" }}>
      <TheorySection 
        title="Reverse Array"
        definition="Inverting the sequence of elements in an array such that the first element becomes the last, the second becomes the second-to-last, and so on."
        timeComplexity="O(N)"
        spaceComplexity="O(1) (In-place)"
        keyPoints={[
          "Two-pointer swap technique",
          "Iterates only N/2 times",
      ...
      <div style={{ height: 48, background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, display: "flex", alignItems: "center", padding: "0 16px", gap: 12 }}>
        <div style={{ fontSize: 14, fontWeight: 800, marginRight: 20 }}>REVERSE_LAB</div>
        ...
        <button style={btnStyle} onClick={() => setArray([10, 20, 30, 40, 50, 60])}>Reset Data</button>
      </div>
      ...
      <div style={{ flex: 1, display: "flex" }}>
        ...
        <div style={{ flex: 1, position: "relative" }}>
          ...
          {renderPanel("pointers", "Pointer States", (
          ...
          {renderPanel("logic", "Reversal Logic", (
              ...
              {step.array.map((val, i) => {
                ...
                return (
                  <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                    <div style={{
                      width: 64, height: 64, 
                      background: isActive ? "rgba(240,136,62,0.15)" : COLORS.surface,
                      border: `2px solid ${isActive ? COLORS.orange : COLORS.border}`,
                      borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center",
                      fontSize: 20, fontWeight: 700, color: isActive ? COLORS.orange : COLORS.textWhite,
                      transition: "all 0.3s"
                    }}>{val}</div>
