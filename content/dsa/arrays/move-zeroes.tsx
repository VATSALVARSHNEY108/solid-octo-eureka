...
const CODE = [
  ...
  "    if (arr[read] != 0) {",
  "        swap(arr[read], arr[write++]);",
  "    }",
  "}",
];

export default function MoveZeroesLab() {
  ...
  const steps = useMemo<Step[]>(() => {
    ...
    s.push({
      type: "init", message: "Initialize write pointer at 0. Start reading.",
      line: 0, array: [...arr], write, read: 0,
      panelData: { write, read: 0, status: "Init" }
    });

    for (let read = 0; read < arr.length; read++) {
      s.push({
        type: "check", message: `Read index ${read}: Value ${arr[read]}`,
        line: 2, array: [...arr], write, read,
        panelData: { read, write, val: arr[read] }
      });

      if (arr[read] !== 0) {
        s.push({
          type: "swap", message: `Non-zero ${arr[read]} found. Swap with write index ${write}.`,
          line: 3, array: [...arr], write, read,
          panelData: { action: "Swapping", read, write }
        });
    ...
    s.push({
      type: "done", message: "All non-zero elements moved forward. Order preserved.",
      line: 5, array: [...arr], write: null, read: null,
      panelData: { finalWrite: write, status: "Done" }
    });
  ...
  return (
    <div style={{ width: "100vw", height:"calc(100vh - 124px)", overflowY: "auto", background: COLORS.bg, scrollbarWidth: "none" }}>
      <TheorySection 
        title="Move Zeroes"
        definition="Shifting all zero elements to the end of an array while maintaining the relative order of all non-zero elements."
        timeComplexity="O(N)"
        spaceComplexity="O(1)"
        keyPoints={[
      ...
      <div style={{ height: 48, background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, display: "flex", alignItems: "center", padding: "0 16px", gap: 12 }}>
        <div style={{ fontSize: 14, fontWeight: 800, marginRight: 20 }}>MOVE_ZEROES_LAB</div>
        ...
        <button style={btnStyle} onClick={() => setArray([0, 1, 0, 3, 12])}>Reset Data</button>
      </div>
      ...
      <div style={{ flex: 1, display: "flex" }}>
        ...
        <div style={{ flex: 1, position: "relative" }}>
          ...
          {renderPanel("pointers", "Pointer State", (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
                <span style={{ color: COLORS.textMuted }}>READ</span>
                <span style={{ color: COLORS.blue, fontWeight: 800 }}>{step.read ?? "N/A"}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
                <span style={{ color: COLORS.textMuted }}>WRITE</span>
                <span style={{ color: COLORS.orange, fontWeight: 800 }}>{step.write ?? "N/A"}</span>
              </div>
            </div>
          ))}
              ...
              {step.array.map((val, i) => {
                ...
                return (
                  <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                    <div style={{
                      width: 64, height: 64, 
                      background: isRead ? "rgba(88,166,255,0.15)" : isWrite ? "rgba(240,136,62,0.15)" : COLORS.surface,
                      border: `2px solid ${isRead ? COLORS.blue : isWrite ? COLORS.orange : COLORS.border}`,
                      borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center",
                      fontSize: 20, fontWeight: 700, color: val === 0 ? COLORS.textDark : COLORS.textWhite,
                      transition: "all 0.3s"
                    }}>{val}</div>
                    <div style={{ fontSize: 12, color: COLORS.textDark }}>{isRead ? "RD" : isWrite ? "WR" : `[${i}]`}</div>
                  </div>
                );
          })}
            </div>
