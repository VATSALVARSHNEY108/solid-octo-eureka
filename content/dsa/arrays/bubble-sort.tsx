...
export default function BubbleSortLab() {
  const [array, setArray] = useState<number[]>([45, 12, 33, 8, 20]);
  ...
  const steps = useMemo<Step[]>(() => {
    ...
    s.push({
      type: "init", message: "Starting Bubble Sort. Heavy elements will 'bubble' to the end.",
      line: 0, array: [...arr], i: null, j: null, swapped: false,
      panelData: { status: "Init", passes: 0 }
    });

    for (let i = 0; i < n - 1; i++) {
      for (let j = 0; j < n - i - 1; j++) {
        s.push({
          type: "compare", message: `Comparing arr[${j}] (${arr[j]}) and arr[${j+1}] (${arr[j+1]}).`,
          line: 2, array: [...arr], i, j, swapped: false,
          panelData: { i, j, comparing: `${arr[j]} > ${arr[j+1]}` }
        });

        if (arr[j] > arr[j + 1]) {
          [arr[j], arr[j + 1]] = [arr[j + 1], arr[j]];
          s.push({
            type: "swap", message: `Swapping ${arr[j+1]} and ${arr[j]}.`,
            line: 3, array: [...arr], i, j, swapped: true,
            panelData: { action: "Swapped", j, next_j: j + 1 }
          });
        }
      }
    }
    ...
    s.push({
      type: "done", message: "Array is fully sorted.",
      line: 6, array: [...arr], i: null, j: null, swapped: false,
      panelData: { status: "Sorted", complexity: "O(n²)" }
    });
  ...
  return (
    <div style={{ width: "100vw", height:"calc(100vh - 124px)", overflowY: "auto", background: COLORS.bg, scrollbarWidth: "none" }}>
      ...
      <div style={{ height: 48, background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, display: "flex", alignItems: "center", padding: "0 16px", gap: 12 }}>
        <div style={{ fontSize: 14, fontWeight: 800, marginRight: 20 }}>BUBBLE_SORT_LAB</div>
        ...
        <button style={btnStyle} onClick={() => setArray(Array.from({length:5}, () => Math.floor(Math.random()*99)))}>Randomize</button>
      </div>
      ...
      <div style={{ flex: 1, display: "flex" }}>
        ...
        <div style={{ flex: 1, position: "relative" }}>
          ...
          {renderPanel("logic", "Bubble Logic", (
          ...
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <div style={{ display: "flex", gap: 12 }}>
              {step.array.map((val, i) => {
                const isComparing = i === step.j || i === (step.j !== null ? step.j + 1 : null);
                const isSorted = step.i !== null && i >= step.array.length - step.i;
                
                return (
                  <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                    <div style={{
                      width: 64, height: 64, 
                      background: isComparing ? "rgba(240,136,62,0.15)" : isSorted ? "rgba(63,185,80,0.15)" : COLORS.surface,
                      border: `2px solid ${isComparing ? COLORS.orange : isSorted ? COLORS.green : COLORS.border}`,
                      borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center",
                      fontSize: 20, fontWeight: 700, color: isComparing ? COLORS.orange : isSorted ? COLORS.green : COLORS.textWhite,
                      transition: "all 0.3s"
                    }}>{val}</div>
                    <div style={{ fontSize: 10, color: COLORS.textDark }}>{isComparing ? "J" : isSorted ? "✓" : `[${i}]`}</div>
                  </div>
                );
          })}
            </div>
