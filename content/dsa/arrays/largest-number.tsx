...
const CODE = [
  "sort(nums, (a, b) => (b + a).compareTo(a + b));",
...
export default function LargestNumberLab() {
  const [nums, setNums] = useState<number[]>([3, 30, 34, 5, 9]);
  ...
  const steps = useMemo<Step[]>(() => {
    const s: Step[] = [];
    const currentNums = nums.map(String);

    s.push({
      type: "init", message: "Goal: Arrange numbers to form the largest possible string. Custom sort based on concat order.",
      line: 0, nums: [...currentNums], i: null, j: null,
      panelData: { status: "Init", count: nums.length }
    });

    // Bubble sort to visualize the comparison logic clearly
    for (let i = 0; i < currentNums.length; i++) {
      for (let j = 0; j < currentNums.length - i - 1; j++) {
        const a = currentNums[j];
        const b = currentNums[j + 1];
        const order1 = a + b;
        const order2 = b + a;
        const swapNeeded = order2 > order1;

        s.push({
          type: "compare", message: `Comparing "${a}" and "${b}". Is "${b}+${a}" (${order2}) > "${a}+${b}" (${order1})?`,
          line: 0, nums: [...currentNums], i: j, j: j + 1,
          panelData: { a, b, order1, order2, swap: swapNeeded }
        });

        if (swapNeeded) {
          [currentNums[j], currentNums[j + 1]] = [currentNums[j + 1], currentNums[j]];
          s.push({
            type: "swap", message: `"${order2}" > "${order1}". Swapping elements.`,
            line: 0, nums: [...currentNums], i: j, j: j + 1,
            panelData: { status: "Swapped", a, b }
          });
        }
      }
    }
    ...
    if (currentNums[0] === "0") {
      s.push({
        type: "zero", message: "Largest number is '0'. Handle edge case.",
        line: 1, nums: ["0"], i: 0, j: null,
        panelData: { result: "0" }
      });
    } else {
      s.push({
        type: "done", message: `Final largest number formed by concatenating all.`,
        line: 4, nums: [...currentNums], i: null, j: null,
        panelData: { result: currentNums.join(""), status: "Done" }
      });
    }
  ...
  return (
    <div style={{ width: "100vw", height:"calc(100vh - 124px)", overflowY: "auto", background: COLORS.bg, scrollbarWidth: "none" }}>
      <TheorySection 
        ...
        definition="Given a list of non-negative integers, arrange them such that they form the largest possible number."
        timeComplexity="O(N log N)"
        spaceComplexity="O(N)"
        keyPoints={[
          "Custom string concatenation sort",
          "Compare (a+b) with (b+a)",
          "Sorting ensures local optimal choice",
          "Handle '0' edge cases carefully"
        ]}
      />
      <div style={{
      ...
      <div style={{ height: 48, background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, display: "flex", alignItems: "center", padding: "0 16px", gap: 12 }}>
        <div style={{ fontSize: 14, fontWeight: 800, marginRight: 20 }}>LARGEST_NUMBER_LAB</div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: COLORS.bg, padding: "4px 12px", borderRadius: 6, border: `1px solid ${COLORS.border}` }}>
          <span style={{ fontSize: 10, color: COLORS.textMuted }}>NUMS:</span>
          <input 
            type="text" 
      ...
      <div style={{ flex: 1, display: "flex" }}>
        ...
        <div style={{ flex: 1, position: "relative" }}>
          ...
          {renderPanel("compare", "Comparison Tracker", (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
                <span style={{ color: COLORS.textMuted }}>A + B</span>
                <span style={{ color: COLORS.blue }}>{step.panelData.order1 ?? "-"}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
                <span style={{ color: COLORS.textMuted }}>B + A</span>
                <span style={{ color: COLORS.orange }}>{step.panelData.order2 ?? "-"}</span>
              </div>
              <div style={{ fontSize: 10, color: step.panelData.swap ? COLORS.orange : COLORS.green, textAlign: "right", marginTop: 4 }}>
                {step.panelData.swap ? "SWAP NEEDED" : "ORDER OK"}
              </div>
            </div>
          ))}

          {renderPanel("logic", "Concatenation Logic", (
          ...
          <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 60 }}>
            <div style={{ display: "flex", gap: 12 }}>
              {step.nums.map((val, idx) => {
                const isActive = idx === step.i || idx === step.j;
                return (
                  <div key={idx} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                    <div style={{
                      ...
                      border: `2px solid ${isActive ? COLORS.blue : COLORS.border}`,
                      borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center",
                      fontSize: 18, fontWeight: 700, color: COLORS.textWhite,
                      transition: "all 0.3s"
                    }}>{val}</div>
                    <div style={{ fontSize: 10, color: COLORS.textDark }}>[{idx}]</div>
                  </div>
                );
          })}
            </div>
            ...
            <div style={{ textAlign: "center" }}>
              <div style={labelStyle}>Resulting Number</div>
              <div style={{ fontSize: 32, fontWeight: 800, color: COLORS.blue, letterSpacing: 2 }}>
                {step.type === "done" ? step.nums.join("") : step.type === "init" ? "?" : step.nums.join("") + "..."}
              </div>
            </div>
          </div>
        </div>
