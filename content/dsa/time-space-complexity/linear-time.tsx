...
import { Clock, Zap, Target, Activity, ShieldCheck, Sparkles, Database, Layers, TrendingUp, AlertTriangle, ArrowDown, Shield, Lock, Repeat, AlertCircle, Minimize, XCircle, Check } from "lucide-react";
import MinimalSimulationStudio from "@/components/MinimalSimulationStudio";

export default function ONLinearTimeLesson() {
  const highlights = [
    { 
      title: "Direct Growth", 
      desc: "Linear relationship between input size and processing time.",
      icon: <TrendingUp size={18} className="text-amber-500" />,
      bg: "rgba(245, 158, 11, 0.05)"
    },
    { 
      title: "Single Pass", 
      desc: "Typical of 'for' loops that visit every element once.",
      icon: <Repeat size={18} className="text-indigo-500" />,
      bg: "rgba(99, 102, 241, 0.05)"
    },
    { 
      title: "Efficiency", 
      desc: "Generally considered highly efficient for large datasets.",
  ...
  return (
    <div style={{ fontVariantNumeric: "tabular-nums", fontFamily: "'DM Sans', sans-serif", color: "var(--text-primary)", padding: "54px" }}>
      ...
      <h2 style={{ 
        ...
        O(N) <span style={{ color: "#8b5cf6" }}>Linear Time</span>
      </h2>
      
      <p style={{ color: "var(--text-secondary)", lineHeight: 1.7, fontSize: 15, maxWidth: 600, marginBottom: 32 }}>
        Execution time grows proportionally with input size. If input doubles, the time taken also doubles. Common in single-pass traversals.
      </p>
      ...
      <MinimalSimulationStudio 
        type="complexity"
        title="O(N)"
        code={["Iterate from 0 to N", "Process current element", "Increment counter", "Loop finishes at N", "Total ops: N"]}
      />
