...
export default function O2NExponentialTimeLesson() {
  const highlights = [
    { 
      title: "Explosive Growth", 
      desc: "Time grows incredibly fast even for tiny N.",
      icon: <Activity size={18} className="text-amber-500" />,
      bg: "rgba(245, 158, 11, 0.05)"
    },
    { 
      title: "All Subsets", 
      desc: "Generating all possible subsets of a set takes 2^N.",
      icon: <Layers size={18} className="text-indigo-500" />,
      bg: "rgba(99, 102, 241, 0.05)"
    },
    { 
      title: "Unfeasible", 
      desc: "Avoid in production unless N is very small (< 30).",
      icon: <XCircle size={18} className="text-emerald-500" />,
  ...
  return (
    <div style={{ fontVariantNumeric: "tabular-nums", fontFamily: "'DM Sans', sans-serif", color: "var(--text-primary)", padding: "54px" }}>
      ...
      <h2 style={{ 
        ...
        O(2^N) <span style={{ color: "#8b5cf6" }}>Exponential Time</span>
      </h2>
      
      <p style={{ color: "var(--text-secondary)", lineHeight: 1.7, fontSize: 15, maxWidth: 600, marginBottom: 32 }}>
        Execution time doubles with every addition to the input size. Extremely slow and usually restricted to small N.
      </p>
      ...
      <MinimalSimulationStudio 
        type="complexity"
        title="O(2^N)"
        code={["Recurse with N-1", "Recurse with N-2", "Combine results", "Ops: 2^N", "Example: Fibonacci"]}
      />
