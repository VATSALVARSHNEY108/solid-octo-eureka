...
export default function ON2QuadraticTimeLesson() {
  const highlights = [
    { 
      title: "Steep Growth", 
      desc: "Small increases in input lead to significant time jumps.",
      icon: <Activity size={18} className="text-amber-500" />,
      bg: "rgba(245, 158, 11, 0.05)"
    },
    { 
      title: "Nested Loops", 
      desc: "Typically 'for i in N' containing 'for j in N'.",
      icon: <Layers size={18} className="text-indigo-500" />,
      bg: "rgba(99, 102, 241, 0.05)"
    },
    { 
      title: "Limit", 
      desc: "Unsuitable for extremely large datasets (N > 10,000).",
  ...
  return (
    <div style={{ fontVariantNumeric: "tabular-nums", fontFamily: "'DM Sans', sans-serif", color: "var(--text-primary)", padding: "54px" }}>
      ...
      <h2 style={{ 
        ...
        O(N²) <span style={{ color: "#8b5cf6" }}>Quadratic Time</span>
      </h2>
      
      <p style={{ color: "var(--text-secondary)", lineHeight: 1.7, fontSize: 15, maxWidth: 600, marginBottom: 32 }}>
        Execution time grows as the square of input size. Often seen in algorithms with nested loops over the same dataset.
      </p>
      ...
      <MinimalSimulationStudio 
        type="complexity"
        title="O(N²)"
        code={["Outer loop: 0 to N", "Inner loop: 0 to N", "Compare elements [i][j]", "Ops: N * N", "Result: O(N²)"]}
      />
