...
function simulateStairs(): StairStep[] {
  const steps: StairStep[] = [];
  const cost = [10, 15, 20];
  const n = cost.length;
  const dp = new Array(n + 1).fill(0);

  steps.push({ type: "init", i: -1, dp: [...dp], message: "Initialize DP array. We want to reach index n (top of the floor).", line: 0 });

  steps.push({ type: "base", i: 0, dp: [...dp], message: "Base case: cost to reach step 0 is 0 (we can start here).", line: 1 });
  steps.push({ type: "base", i: 1, dp: [...dp], message: "Base case: cost to reach step 1 is 0 (we can start here too).", line: 2 });

  for (let i = 2; i <= n; i++) {
    steps.push({ type: "check", i, dp: [...dp], message: `Evaluating cost to reach step ${i}.`, line: 3 });
    
    const oneStepCost = dp[i-1] + cost[i-1];
    const twoStepCost = dp[i-2] + cost[i-2];
    
    dp[i] = Math.min(oneStepCost, twoStepCost);
    
    steps.push({ type: "update", i, dp: [...dp], message: `Min of taking 1 step from ${i-1} (cost ${oneStepCost}) or 2 steps from ${i-2} (cost ${twoStepCost}) is ${dp[i]}.`, line: 4 });
  }

  steps.push({ type: "done", i: -1, dp: [...dp], message: `Done. The minimum cost to reach the top is ${dp[n]}.`, line: 5 });
  return steps;
}

export default function MinCostClimbingStairsLesson() {
  ...
  const steps = useMemo(() => simulateStairs(), []);
  ...
  const codeSnippet = [
    ...
    "// We can start at step 0 or 1 with cost 0",
    "dp[0] = 0;",
    "dp[1] = 0;",
    "for (let i = 2; i <= n; i++) {",
    "  dp[i] = Math.min(",
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <p className="description">You are given an integer array cost where cost[i] is the cost of ith step on a staircase. Pay the cost to climb one or two steps to reach the top.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>This is a direct variation of the classic Fibonacci sequence or "Climbing Stairs" problem, but instead of counting ways, we are minimizing cost.</p></article>
          <article className="guide-card"><h2>The DP State</h2><p>`dp[i]` represents the minimum cost to REACH step `i`. Notice it's the cost to reach it, not the cost to leave it.</p></article>
          <article className="guide-card"><h2>The Recurrence</h2><p>To reach step `i`, you either came from step `i-1` (and paid `cost[i-1]`) or you came from step `i-2` (and paid `cost[i-2]`). Take the minimum.</p></article>
          <article className="guide-card highlight"><h2>Space Optimization</h2><p>Since `dp[i]` only depends on `dp[i-1]` and `dp[i-2]`, we only need two variables to track the state, reducing space complexity to O(1).</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the algorithm evaluate 1-step vs 2-step jumps to minimize the total cost to reach the top (index 3).</span>
              </div>
              <h2>Current Step</h2>
            ...
            <div className="canvas" style={{ display: 'flex', flexDirection: 'column', gap: '20px', padding: '20px' }}>
               ...
               <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', marginBottom: '20px', alignItems: 'flex-end', height: '100px' }}>
                 {cost.map((val, idx) => (
                   <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                     <span style={{ fontSize: '10px', color: 'var(--muted)', marginBottom: '4px' }}>Cost: {val}</span>
                     <div style={{
                       width: '50px', height: `${40 + val}px`, borderRadius: '8px 8px 0 0',
                       background: 'var(--panel2)', border: '2px solid var(--border)', borderBottom: 'none',
                       display: 'flex', alignItems: 'flex-end', justifyContent: 'center', paddingBottom: '10px',
                       fontWeight: 'bold', fontSize: '12px', color: 'var(--muted)'
                     }}>
                       Step {idx}
                     </div>
                   </div>
                 ...
                 <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                   <span style={{ fontSize: '10px', color: 'var(--green)', marginBottom: '4px' }}>TOP</span>
                   <div style={{
                     width: '50px', height: '80px', borderRadius: '8px 8px 0 0',
                     background: 'color-mix(in srgb, var(--green) 10%, transparent)', border: '2px solid var(--green)', borderBottom: 'none',
                     display: 'flex', alignItems: 'flex-end', justifyContent: 'center', paddingBottom: '10px',
                     fontWeight: 'bold', fontSize: '12px', color: 'var(--green)'
                   }}>
                     End
                   </div>
                 </div>
               ...
               <div style={{ textAlign: 'center', color: 'var(--muted)', fontSize: '13px', marginBottom: '8px' }}>DP Array (Min Cost to Reach Step)</div>
