...
function simulateStock(prices: number[]): StockStep[] {
  const steps: StockStep[] = [];
  let minPrice = Infinity;
  let maxProfit = 0;

  steps.push({ type: "init", day: -1, price: 0, minPrice, profit: 0, maxProfit, message: "Initialize minPrice to Infinity and maxProfit to 0.", line: 0 });

  for (let i = 0; i < prices.length; i++) {
    const price = prices[i];
    steps.push({ type: "loop", day: i, price, minPrice, profit: 0, maxProfit, message: `Day ${i + 1}: Check price ${price}`, line: 1 });
    
    if (price < minPrice) {
      minPrice = price;
      steps.push({ type: "update_min", day: i, price, minPrice, profit: 0, maxProfit, message: `Found new minimum price: ${minPrice}`, line: 2 });
    } else {
      steps.push({ type: "check_profit", day: i, price, minPrice, profit: price - minPrice, maxProfit, message: `Check profit if selling today: ${price} - ${minPrice} = ${price - minPrice}`, line: 3 });
      if (price - minPrice > maxProfit) {
        maxProfit = price - minPrice;
        steps.push({ type: "update_max", day: i, price, minPrice, profit: price - minPrice, maxProfit, message: `Found new maximum profit: ${maxProfit}`, line: 4 });
      }
    }
  }

  steps.push({ type: "done", day: prices.length, price: 0, minPrice, profit: 0, maxProfit, message: `Done. Maximum profit is ${maxProfit}`, line: 5 });
  return steps;
}

export default function BestTimeToBuySellStockILesson() {
  ...
  const steps = useMemo(() => simulateStock(prices), [prices]);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          <span className="eyebrow">Dynamic Programming • State Tracking</span>
          <h1>Best Time to Buy and Sell Stock</h1>
          <p className="description">Track the minimum price seen so far to find the maximum possible profit from a single transaction.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>You want to maximize your profit by choosing a single day to buy one stock and choosing a different day in the future to sell that stock.</p></article>
          <article className="guide-card"><h2>State Variable</h2><p>Keep track of the minimum price encountered so far. This allows computing the maximum profit if we sell on the current day.</p></article>
          <article className="guide-card"><h2>Transitions</h2><p>If current price is less than min_price, update min_price. Otherwise, check if current price - min_price is greater than max_profit.</p></article>
          <article className="guide-card highlight"><h2>Key Insight</h2><p>We only need a single pass (O(N) time) because the best buy day for any given sell day is always the minimum price seen before it.</p></article>
        </div>
      </section>
      <section id="simulator" className="simulator">
        <div className="simulator-content">
          <div className="editor" style={{ display: 'flex', gap: '8px', padding: '0 40px' }}>
            <input value={pricesInput} onChange={(e) => setPricesInput(e.target.value)} placeholder="e.g., 7,1,5,3,6,4" style={{ flex: 1, maxWidth: '300px' }} />
            <button onClick={applyPrices}>Update Prices</button>
            <button onClick={() => { setPricesInput("7, 1, 5, 3, 6, 4"); setPrices([7, 1, 5, 3, 6, 4]); }}>Reset</button>
          </div>
              ...
              <div className="simulation-data">
                <div className="data-group">
                  ...
                  <div className="distances">
                    <span className={step.minPrice !== Infinity ? 'active' : ''}>minPrice: {step.minPrice === Infinity ? 'inf' : step.minPrice}</span>
                    <span className={step.maxProfit > 0 ? 'active' : ''} style={{borderColor: 'var(--green)', color: step.maxProfit > 0 ? 'var(--green)' : ''}}>maxProfit: {step.maxProfit}</span>
                  </div>
                </div>
              </div>
