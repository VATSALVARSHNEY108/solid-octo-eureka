...
function simulateUniquePaths(): PathStep[] {
  ...
  steps.push({ type: "init", r: -1, c: -1, dp: dp.map(row => [...row]), message: "Initialize DP grid with 0s.", line: 0 });

  for (let c = 0; c < C; c++) {
    dp[0][c] = 1;
    steps.push({ type: "init_row", r: 0, c, dp: dp.map(row => [...row]), message: "Top row can only be reached by moving Right. Set to 1.", line: 1 });
  }

  for (let r = 1; r < R; r++) {
    dp[r][0] = 1;
    steps.push({ type: "init_col", r, c: 0, dp: dp.map(row => [...row]), message: "Leftmost column can only be reached by moving Down. Set to 1.", line: 2 });
  }

  for (let r = 1; r < R; r++) {
    for (let c = 1; c < C; c++) {
      steps.push({ type: "check", r, c, dp: dp.map(row => [...row]), message: `Calculating paths to (${r}, ${c}). Looking UP and LEFT.`, line: 3 });
      dp[r][c] = dp[r - 1][c] + dp[r][c - 1];
      steps.push({ type: "update", r, c, dp: dp.map(row => [...row]), message: `Paths to (${r}, ${c}) = ${dp[r - 1][c]} + ${dp[r][c - 1]} = ${dp[r][c]}.`, line: 4 });
    }
  }

  steps.push({ type: "done", r: -1, c: -1, dp: dp.map(row => [...row]), message: `Done. Total unique paths to bottom-right is ${dp[R-1][C-1]}.`, line: 5 });
  return steps;
}

export default function UniquePathsLesson() {
  ...
  const steps = useMemo(() => simulateUniquePaths(), []);
  ...
  const codeSnippet = [
    ...
    "// We can init everything to 1 because the edges are 1,",
    "// and we just overwrite the inner cells.",
    "for (let r = 1; r < m; r++) {",
    "  for (let c = 1; c < n; c++) {",
    "    dp[r][c] = dp[r - 1][c] + dp[r][c - 1];",
    "  }",
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>Unique Paths</h1>
          <p className="description">Find the number of possible unique paths from the top-left corner to the bottom-right corner of a grid, moving only Right or Down.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>This is the "Hello World" of 2D grid DP. At any given cell, how did we get here? We must have come from either the cell directly above, or directly to the left.</p></article>
          <article className="guide-card"><h2>The Recurrence</h2><p>Since we can only arrive from UP or LEFT, the total number of unique paths to a cell is simply the sum of the paths to those two source cells: `dp[r][c] = dp[r-1][c] + dp[r][c-1]`.</p></article>
          <article className="guide-card"><h2>Base Cases</h2><p>Any cell in the very first row can only be reached by moving continuously Right. So `dp[0][c] = 1`. Similarly, the first column is all `1`s.</p></article>
          <article className="guide-card highlight"><h2>Math Alternative</h2><p>This problem can also be solved in `O(min(M, N))` time using combinatorics: it's exactly `(M+N-2) Choose (M-1)`. But the DP approach is more extensible to obstacles.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the 2D grid populate with path sums recursively.</span>
              </div>
              <h2>Current Step</h2>
