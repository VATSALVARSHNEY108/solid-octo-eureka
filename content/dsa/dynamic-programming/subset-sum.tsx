...
function simulateSubsetSum(): SubStep[] {
  const steps: SubStep[] = [];
  const nums = [2, 3, 7, 8, 10];
  const target = 11;
  const dp = new Array(target + 1).fill(false);
  dp[0] = true;

  steps.push({ type: "init", i: -1, j: -1, dp: [...dp], message: `Target is ${target}. Initialize 1D DP array of size ${target + 1}. dp[0] is true because a sum of 0 is always possible (empty subset).`, line: 0 });

  for (let i = 0; i < nums.length; i++) {
    steps.push({ type: "outer", i, j: -1, dp: [...dp], message: `Processing number ${nums[i]}. We iterate backwards to avoid reusing the same number.`, line: 1 });
    
    for (let j = target; j >= nums[i]; j--) {
      steps.push({ type: "check", i, j, dp: [...dp], message: `Can we make sum ${j} using ${nums[i]}? Check if dp[${j - nums[i]}] is true.`, line: 2 });
      
      if (dp[j - nums[i]]) {
        dp[j] = true;
        steps.push({ type: "update", i, j, dp: [...dp], message: `Yes! dp[${j - nums[i]}] is true, so setting dp[${j}] to true.`, line: 3 });
      } else {
        steps.push({ type: "skip", i, j, dp: [...dp], message: `dp[${j - nums[i]}] is false. Cannot reach ${j} through this path.`, line: 4 });
      }
    }
  }

  steps.push({ type: "done", i: -1, j: -1, dp: [...dp], message: `Done. dp[${target}] is ${dp[target]}. Thus, subset sum ${target} is ${dp[target] ? "possible" : "impossible"}.`, line: 5 });
  return steps;
}

export default function SubsetSumLesson() {
  const { resolvedTheme } = useTheme();
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);
  const theme = mounted ? ((resolvedTheme as "light" | "dark") || "dark") : "dark";
  ...
  const steps = useMemo(() => simulateSubsetSum(), []);
  ...
  const codeSnippet = [
    ...
    "dp[0] = true;",
    "for (let i = 0; i < nums.length; i++) {",
    "  // Iterate backwards to optimize space to 1D array",
    "  for (let j = target; j >= nums[i]; j--) {",
  ...
  return (
    <main className="page" data-theme={theme} suppressHydrationWarning>
      <section className="hero">
        <div className="content-width">
          <span className="eyebrow">Dynamic Programming • 0/1 Knapsack</span>
          <h1>Subset Sum</h1>
          <p className="description">Determine if there is a subset of the given array whose elements sum up to a specific target. This is a foundational 0/1 Knapsack problem.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>This asks a simple YES/NO question: Can we pick some elements from the array such that they sum exactly to `target`? It's a classic subset selection problem.</p></article>
          <article className="guide-card"><h2>The DP Array</h2><p>We use a 1D boolean array where `dp[j]` is `true` if a subset with sum `j` exists. It's initialized to `false`, except `dp[0] = true`.</p></article>
          <article className="guide-card"><h2>The Transition</h2><p>For each number `num`, if we can reach sum `j - num`, then we can definitely reach sum `j` by just adding `num`. So, `dp[j] = dp[j] || dp[j - num]`.</p></article>
          <article className="guide-card highlight"><h2>Reverse Loop Trap</h2><p>When compressing from a 2D matrix down to a 1D array, you MUST iterate the inner loop backwards. If you iterate forwards, you might accidentally use the same `num` multiple times!</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch how the reachable sums ripple through the boolean array from right to left.</span>
              </div>
              <h2>Current Step</h2>
