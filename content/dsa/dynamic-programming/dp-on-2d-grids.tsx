...
function simulateGridDP(): GridStep[] {
  const steps: GridStep[] = [];
  const R = 3, C = 3;
  const dp = Array.from({ length: R }, () => Array(C).fill(0));
  
  steps.push({ type: "init", r: -1, c: -1, dp: dp.map(row => [...row]), message: "Initialize a 3x3 DP table. We start from (0,0).", line: 0 });

  dp[0][0] = 1;
  steps.push({ type: "base", r: 0, c: 0, dp: dp.map(row => [...row]), message: "dp[0][0] = 1 (1 way to be at the start).", line: 1 });

  for (let r = 0; r < R; r++) {
    for (let c = 0; c < C; c++) {
      if (r === 0 && c === 0) continue;
      steps.push({ type: "check", r, c, dp: dp.map(row => [...row]), message: `Calculating paths to cell (${r}, ${c}).`, line: 2 });
      
      const waysFromTop = r > 0 ? dp[r - 1][c] : 0;
      const waysFromLeft = c > 0 ? dp[r][c - 1] : 0;
      
      steps.push({ type: "calc", r, c, dp: dp.map(row => [...row]), message: `Ways from top: ${waysFromTop}, Ways from left: ${waysFromLeft}.`, line: 3 });
      
      dp[r][c] = waysFromTop + waysFromLeft;
      steps.push({ type: "update", r, c, dp: dp.map(row => [...row]), message: `dp[${r}][${c}] = ${dp[r][c]}.`, line: 4 });
    }
  }

  steps.push({ type: "done", r: R - 1, c: C - 1, dp: dp.map(row => [...row]), message: `Done. Total unique paths to bottom-right is ${dp[R - 1][C - 1]}.`, line: 5 });
  return steps;
}

export default function DPOn2DGridsLesson() {
  ...
  const steps = useMemo(() => simulateGridDP(), []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>DP on 2D Grids</h1>
          <p className="description">Explore how to solve grid traversal problems using dynamic programming by analyzing state transitions from adjacent cells.</p>
          <div className="complexity-tag-group">
            <div className="complexity-tag"><span className="label">Time</span><span className="value">O(R * C)</span></div>
            <div className="complexity-tag"><span className="label">Space</span><span className="value">O(R * C)</span></div>
          </div>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>Grid DP is extremely common. The state `dp[r][c]` typically represents the cost, max profit, or number of ways to reach cell `(r, c)`.</p></article>
          <article className="guide-card"><h2>Directional Constraints</h2><p>Since we usually can only move Right or Down, we can guarantee that when calculating `dp[r][c]`, both `dp[r-1][c]` and `dp[r][c-1]` are already computed.</p></article>
          <article className="guide-card"><h2>Base Cases</h2><p>The starting cell `(0, 0)` is initialized based on the problem (e.g. `1` for path counting, `grid[0][0]` for path sum). Boundaries need bounds checking.</p></article>
          <article className="guide-card highlight"><h2>Space Optimization</h2><p>Because `dp[r][c]` only depends on the current row and the previous row, we can often optimize the O(R * C) space down to O(C) by keeping a 1D array.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the 3x3 DP grid evaluate paths cell by cell.</span>
              </div>
              <h2>Current Step</h2>
               ...
               <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 60px)', gap: '8px', justifyContent: 'center' }}>
                 {step.dp.map((row, r) => 
                   row.map((val, c) => {
                     ...
                     const isSource = step.r !== -1 && ((step.r - 1 === r && step.c === c) || (step.r === r && step.c - 1 === c));
