...
function simulateWildcard(): WildcardStep[] {
  ...
  steps.push({ type: "init", i: -1, j: -1, dp: dp.map(r => [...r]), message: "Initialize DP table. dp[i][j] is true if s[0...i-1] matches p[0...j-1].", line: 0 });

  dp[0][0] = true;
  steps.push({ type: "base_00", i: 0, j: 0, dp: dp.map(r => [...r]), message: "Empty string matches empty pattern. dp[0][0] = true.", line: 1 });

  for (let j = 1; j <= M; j++) {
    if (p[j - 1] === '*') {
      dp[0][j] = dp[0][j - 1];
      steps.push({ type: "base_star", i: 0, j, dp: dp.map(r => [...r]), message: `Pattern is '*'. It can match empty sequence if everything before it matched empty. dp[0][${j}] = ${dp[0][j]}.`, line: 2 });
    } else {
      steps.push({ type: "base_char", i: 0, j, dp: dp.map(r => [...r]), message: `Pattern is '${p[j-1]}'. Cannot match empty string. dp[0][${j}] = false.`, line: 3 });
    }
  }

  for (let i = 1; i <= N; i++) {
    for (let j = 1; j <= M; j++) {
      steps.push({ type: "check", i, j, dp: dp.map(r => [...r]), message: `Comparing s[${i-1}]='${s[i-1]}' with p[${j-1}]='${p[j-1]}'.`, line: 4 });
      
      if (p[j - 1] === s[i - 1] || p[j - 1] === '?') {
        dp[i][j] = dp[i - 1][j - 1];
        steps.push({ type: "match_char", i, j, dp: dp.map(r => [...r]), message: `Match! Result depends on previous characters. dp[${i}][${j}] = dp[${i-1}][${j-1}] = ${dp[i][j]}.`, line: 5 });
      } else if (p[j - 1] === '*') {
        dp[i][j] = dp[i - 1][j] || dp[i][j - 1];
        steps.push({ type: "match_star", i, j, dp: dp.map(r => [...r]), message: `Star! Match empty (dp[${i}][${j-1}]) OR match current char (dp[${i-1}][${j}]). Result: ${dp[i][j]}.`, line: 6 });
      } else {
        steps.push({ type: "mismatch", i, j, dp: dp.map(r => [...r]), message: `Mismatch. dp[${i}][${j}] remains false.`, line: 7 });
      }
    }
  }

  steps.push({ type: "done", i: N, j: M, dp: dp.map(r => [...r]), message: `Done. Final answer is dp[${N}][${M}] = ${dp[N][M]}.`, line: 8 });
  return steps;
}

export default function WildcardMatchingLesson() {
  ...
  const steps = useMemo(() => simulateWildcard(), []);
  ...
  const codeSnippet = [
    ...
    "dp[0][0] = true;",
    "// Base case: matching empty string with '*'",
    "for (let j = 1; j <= M; j++) {",
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <p className="description">Given an input string (s) and a pattern (p), implement wildcard pattern matching with support for '?' and '*'.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>This is a classic 2D DP problem. The `?` matches any single character. The `*` is the tricky part—it matches ANY sequence of characters (including the empty sequence).</p></article>
          <article className="guide-card"><h2>The DP Table</h2><p>`dp[i][j]` is `true` if the first `i` characters of `s` match the first `j` characters of `p`. We pad the strings with a 1-indexed approach to handle empty string base cases elegantly.</p></article>
          <article className="guide-card"><h2>The Exact Match ('?')</h2><p>If the characters match exactly, or if the pattern character is `?`, then `dp[i][j]` simply takes the value from diagonally above-left: `dp[i-1][j-1]`.</p></article>
          <article className="guide-card highlight"><h2>The Star Power ('*')</h2><p>If the pattern is `*`, it can either act as empty (`dp[i][j-1]`), or it can swallow the current character of `s` and remain active for the next (`dp[i-1][j]`).</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch how the truth values propagate. Pay close attention to how '*' allows branching.</span>
              </div>
              <h2>Current Step</h2>
               ...
               <div style={{ display: 'flex', justifyContent: 'center' }}>
                 <table style={{ borderCollapse: 'collapse', textAlign: 'center' }}>
                   ...
                   <tbody>
                     {step.dp?.map((row, i) => (
                       <tr key={i}>
                         <th style={{ padding: '8px', borderRight: '1px solid var(--border)', color: step.i === i ? 'var(--amber)' : 'var(--text)' }}>
                           {i === 0 ? '""' : `'${s[i-1]}'`}
                         </th>
                         {row.map((val, j) => {
                           const isActive = step.i === i && step.j === j;
                           let isSource = false;
                           if (step.type === "match_char" && step.i - 1 === i && step.j - 1 === j) isSource = true;
                           if (step.type === "match_star" && ((step.i === i && step.j - 1 === j) || (step.i - 1 === i && step.j === j))) isSource = true;
                           
                           let bg = val ? 'color-mix(in srgb, var(--green) 15%, transparent)' : 'transparent';
                           ...
                           return (
                             <td key={j} style={{
                               ...
                               <div style={{
                                 ...
                                 background: bg, border: `1px solid ${border}`, fontSize: '12px', fontWeight: val ? 'bold' : 'normal',
