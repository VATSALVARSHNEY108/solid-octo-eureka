...
// Very similar to DP on Strings, since LCS is the primary example.
// Let's create a slightly different visual focus, perhaps highlighting the trace-back.
...
function simulateLCS(): LCSStep[] {
  ...
  steps.push({ type: "init", i: -1, j: -1, dp: dp.map(r => [...r]), message: "Initialize LCS DP table with zeros.", line: 0 });

  for (let i = 1; i <= m; i++) {
    for (let j = 1; j <= n; j++) {
      if (s1[i-1] === s2[j-1]) {
        dp[i][j] = 1 + dp[i-1][j-1];
        steps.push({ type: "match", i, j, dp: dp.map(r => [...r]), message: `Match '${s1[i-1]}'! Diagonally inherit and add 1 => ${dp[i][j]}.`, line: 1 });
      } else {
        dp[i][j] = Math.max(dp[i-1][j], dp[i][j-1]);
        steps.push({ type: "no_match", i, j, dp: dp.map(r => [...r]), message: `Mismatch. Max of Top (${dp[i-1][j]}) or Left (${dp[i][j-1]}) => ${dp[i][j]}.`, line: 2 });
      }
    }
  }

  steps.push({ type: "done", i: -1, j: -1, dp: dp.map(r => [...r]), message: `Done. The Longest Common Subsequence length is ${dp[m][n]}.`, line: 3 });
  return steps;
}

export default function LongestCommonSubsequenceLesson() {
  ...
  const steps = useMemo(() => simulateLCS(), []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <p className="description">Find the length of the longest subsequence common to two strings. A foundational DP problem.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>LCS asks you to find the longest common sequence of characters that appear in the same relative order in both strings, but not necessarily contiguously.</p></article>
          <article className="guide-card"><h2>The Logic</h2><p>If characters match, the sequence length increases by 1 relative to the sequences before both characters. If not, the sequence is the max of ignoring the character from string A or string B.</p></article>
          <article className="guide-card"><h2>The Grid</h2><p>We build an `(M+1) x (N+1)` grid where `dp[i][j]` represents the LCS of prefixes `text1[0..i-1]` and `text2[0..j-1]`.</p></article>
          <article className="guide-card highlight"><h2>Retrieving the String</h2><p>You can trace back from `dp[M][N]` to find the actual string. If chars matched, move diagonal-up-left. Otherwise, move to the max of Top or Left.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the LCS grid populate. Notice the diagonal jumps on matches.</span>
              </div>
              <h2>Current Step</h2>
            ...
            <div className="canvas" style={{ display: 'flex', flexDirection: 'column', gap: '20px', padding: '20px' }}>
               ...
               <div style={{ display: 'flex', justifyContent: 'center' }}>
                 <table style={{ borderCollapse: 'collapse' }}>
                   <thead>
                     <tr>
                       <th style={{ padding: '8px', borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>T1 \ T2</th>
