...
function simulatePrintLCS(): TraceStep[] {
  ...
  steps.push({ type: "init", i, j, result, message: "DP table pre-calculated. Start backtracking from bottom-right (dp[5][3]).", line: 0 });

  while (i > 0 && j > 0) {
    steps.push({ type: "check", i, j, result, message: `Checking s1[${i-1}] ('${s1[i-1]}') and s2[${j-1}] ('${s2[j-1]}').`, line: 1 });
    
    if (s1[i - 1] === s2[j - 1]) {
      result = s1[i - 1] + result; // Prepend to result
      steps.push({ type: "match", i, j, result, message: `Match found! Prepend '${s1[i-1]}' to result. Move diagonally to dp[${i-1}][${j-1}].`, line: 2 });
      i--;
      j--;
    } else {
      if (dp[i - 1][j] > dp[i][j - 1]) {
        steps.push({ type: "move_up", i, j, result, message: `Mismatch. dp[${i-1}][${j}] (${dp[i-1][j]}) > dp[${i}][${j-1}] (${dp[i][j-1]}). Move UP.`, line: 3 });
        i--;
      } else {
        steps.push({ type: "move_left", i, j, result, message: `Mismatch. Move LEFT to the larger (or equal) value dp[${i}][${j-1}] (${dp[i][j-1]}).`, line: 4 });
        j--;
      }
    }
  }

  steps.push({ type: "done", i, j, result, message: `Hit boundary (0). Traceback complete. The LCS is "${result}".`, line: 5 });
  return steps;
}

export default function PrintLCSLesson() {
  ...
  const steps = useMemo(() => simulatePrintLCS(), []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>Printing the Longest Common Subsequence</h1>
          <p className="description">After populating the LCS DP table, learn how to reconstruct the actual string instead of just returning the max length.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>Populating a DP table tells you the length of the optimal solution. Tracing backward through that table tells you the *elements* of that optimal solution.</p></article>
          <article className="guide-card"><h2>The Starting Point</h2><p>You always start your traceback from the final computed answer, which is at the bottom-right corner of the DP matrix: `dp[m][n]`.</p></article>
          <article className="guide-card"><h2>The Logic</h2><p>If characters match, that character is part of the LCS. Prepend it to the result and move diagonally UP-LEFT (`i-1, j-1`).</p></article>
          <article className="guide-card highlight"><h2>Mismatches</h2><p>If characters don't match, we know the LCS must have come from either the cell ABOVE us or to the LEFT of us. We simply move to whichever has the larger value.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the traceback algorithm walk backward from the bottom right to reconstruct the string.</span>
              </div>
              <h2>Current Step</h2>
              ...
              <div style={{ marginTop: '20px', padding: '16px', background: 'var(--panel2)', borderRadius: '8px', border: '2px solid var(--green)' }}>
                <span style={{ fontSize: '12px', color: 'var(--muted)', textTransform: 'uppercase' }}>Constructed String</span>
                <div style={{ fontSize: '24px', fontWeight: 'bold', color: 'var(--green)', minHeight: '30px', letterSpacing: '4px' }}>
                  {step.result}
                </div>
              </div>
            </aside>
