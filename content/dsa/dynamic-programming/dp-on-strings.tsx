...
function simulateStringDP(): StringStep[] {
  ...
  steps.push({ type: "init", i: -1, j: -1, dp: dp.map(r => [...r]), message: "Initialize DP table for Longest Common Subsequence (LCS).", line: 0 });
  ...
  for (let i = 1; i <= m; i++) {
    for (let j = 1; j <= n; j++) {
      ...
      if (s1[i-1] === s2[j-1]) {
        dp[i][j] = 1 + dp[i-1][j-1];
        steps.push({ type: "match", i, j, dp: dp.map(r => [...r]), message: `Match! dp[${i}][${j}] = 1 + dp[${i-1}][${j-1}] = ${dp[i][j]}.`, line: 2 });
      } else {
        dp[i][j] = Math.max(dp[i-1][j], dp[i][j-1]);
        steps.push({ type: "no_match", i, j, dp: dp.map(r => [...r]), message: `No Match. dp[${i}][${j}] = max(dp[${i-1}][${j}], dp[${i}][${j-1}]) = ${dp[i][j]}.`, line: 3 });
      }
    }
  }

  steps.push({ type: "done", i: -1, j: -1, dp: dp.map(r => [...r]), message: `Done. The LCS length is ${dp[m][n]}.`, line: 4 });
  return steps;
}

export default function DPOnStringsLesson() {
  ...
  const steps = useMemo(() => simulateStringDP(), []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>DP on Strings</h1>
          <p className="description">String matching problems often rely on comparing two strings recursively. We use a 2D grid to track the relationships.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>LCS (Longest Common Subsequence) is the mother of all string DP problems. Edit Distance, Distinct Subsequences, etc. are derived from this pattern.</p></article>
          <article className="guide-card"><h2>State Definition</h2><p>The state `dp[i][j]` always refers to the relationship between the prefix `s1[0..i-1]` and `s2[0..j-1]`.</p></article>
          <article className="guide-card"><h2>Match Case</h2><p>If the characters match, we typically extend the sequence: `dp[i][j] = 1 + dp[i-1][j-1]`.</p></article>
          <article className="guide-card highlight"><h2>No-Match Case</h2><p>If they don't match, we branch out and find the optimal combination by ignoring the current char from `s1` or `s2`.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the LCS DP logic trace "abc" and "ac".</span>
              </div>
              <h2>Current Step</h2>
