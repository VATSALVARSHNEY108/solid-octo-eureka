...
function simulateStockIII(prices: number[]): StockStep[] {
  ...
  steps.push({ type: "init", day: -1, price: 0, buy1, sell1, buy2, sell2, message: "Initialize buy states to -Infinity and sell states to 0.", line: 0 });

  for (let i = 0; i < prices.length; i++) {
    const price = prices[i];
    steps.push({ type: "loop", day: i, price, buy1, sell1, buy2, sell2, message: `Day ${i + 1}: Check price ${price}`, line: 1 });
    
    const prevBuy1 = buy1;
    buy1 = Math.max(buy1, -price);
    steps.push({ type: "buy1", day: i, price, buy1, sell1, buy2, sell2, message: `buy1 = max(${prevBuy1 === -Infinity ? '-inf' : prevBuy1}, -${price}) = ${buy1}`, line: 2 });
    
    const prevSell1 = sell1;
    sell1 = Math.max(sell1, buy1 + price);
    steps.push({ type: "sell1", day: i, price, buy1, sell1, buy2, sell2, message: `sell1 = max(${prevSell1}, ${buy1} + ${price}) = ${sell1}`, line: 3 });

    const prevBuy2 = buy2;
    buy2 = Math.max(buy2, sell1 - price);
    steps.push({ type: "buy2", day: i, price, buy1, sell1, buy2, sell2, message: `buy2 = max(${prevBuy2 === -Infinity ? '-inf' : prevBuy2}, ${sell1} - ${price}) = ${buy2}`, line: 4 });

    const prevSell2 = sell2;
    sell2 = Math.max(sell2, buy2 + price);
    steps.push({ type: "sell2", day: i, price, buy1, sell1, buy2, sell2, message: `sell2 = max(${prevSell2}, ${buy2} + ${price}) = ${sell2}`, line: 5 });
  }

  steps.push({ type: "done", day: prices.length, price: 0, buy1, sell1, buy2, sell2, message: `Done. Maximum profit (at most 2 transactions) is ${sell2}`, line: 6 });
  return steps;
}

export default function BestTimeToBuySellStockIIILesson() {
  const { resolvedTheme } = useTheme();
  const theme = (resolvedTheme as "light" | "dark") || "dark";

  const [pricesInput, setPricesInput] = useState("3, 3, 5, 0, 0, 3, 1, 4");
  ...
  const steps = useMemo(() => simulateStockIII(prices), [prices]);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <p className="description">Maximize your profit given you can complete at most two transactions.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>You can complete at most two transactions. We model this as a state machine with 4 states: after 1st buy, after 1st sell, after 2nd buy, after 2nd sell.</p></article>
          <article className="guide-card"><h2>State Transitions</h2><p>Each state can either stay as is (don't transact today) or transition to the new state by transacting at today's price.</p></article>
          <article className="guide-card"><h2>DP Table Compression</h2><p>Since the state on day i only depends on the state on day i-1, we can compress the DP table into just 4 variables.</p></article>
          <article className="guide-card highlight"><h2>Key Insight</h2><p>The states are updated sequentially. We can safely update them in place because the updated previous state will only increase our current state's potential.</p></article>
        </div>
      </section>
      <section id="simulator" className="simulator">
        <div className="simulator-content">
          <div className="editor" style={{ display: 'flex', gap: '8px', padding: '0 40px' }}>
            <input value={pricesInput} onChange={(e) => setPricesInput(e.target.value)} placeholder="e.g., 3,3,5,0,0,3,1,4" style={{ flex: 1, maxWidth: '300px' }} />
            <button onClick={applyPrices}>Update Prices</button>
            <button onClick={() => { setPricesInput("3, 3, 5, 0, 0, 3, 1, 4"); setPrices([3, 3, 5, 0, 0, 3, 1, 4]); }}>Reset</button>
          </div>
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>🖱️ Adjust the array of prices above to test different scenarios.</span>
              ...
              <div className="simulation-data">
                <div className="data-group">
                  ...
                  <div className="distances">
                    <span className={step.buy1 !== -Infinity ? 'active' : ''}>buy1: {fmt(step.buy1)}</span>
                    <span className={step.sell1 > 0 ? 'active' : ''} style={{borderColor: step.sell1 > 0 ? 'var(--green)' : '', color: step.sell1 > 0 ? 'var(--green)' : ''}}>sell1: {step.sell1}</span>
                    <span className={step.buy2 !== -Infinity ? 'active' : ''}>buy2: {fmt(step.buy2)}</span>
                    <span className={step.sell2 > 0 ? 'active' : ''} style={{borderColor: step.sell2 > 0 ? 'var(--blue)' : '', color: step.sell2 > 0 ? 'var(--blue)' : ''}}>sell2: {step.sell2}</span>
                  </div>
                </div>
              </div>
