...
function simulateKnapsack(): KnapsackStep[] {
  ...
  steps.push({ type: "init", i: -1, w: -1, dp: dp.map(r => [...r]), message: "Initialize DP table. Rows: Items, Cols: Capacities 0 to W.", line: 0 });

  for (let i = 1; i <= n; i++) {
    for (let w = 1; w <= W; w++) {
      steps.push({ type: "check", i, w, dp: dp.map(r => [...r]), message: `Item ${i} (wt:${wt[i-1]}, val:${val[i-1]}) at capacity ${w}.`, line: 1 });
      
      if (wt[i-1] <= w) {
        const skip = dp[i-1][w];
        const pick = val[i-1] + dp[i-1][w - wt[i-1]];
        dp[i][w] = Math.max(skip, pick);
        steps.push({ type: "pick_skip", i, w, dp: dp.map(r => [...r]), message: `Can fit! Max of Skip (${skip}) or Pick (${val[i-1]} + ${dp[i-1][w - wt[i-1]]} = ${pick}) => ${dp[i][w]}.`, line: 2 });
      } else {
        dp[i][w] = dp[i-1][w];
        steps.push({ type: "skip", i, w, dp: dp.map(r => [...r]), message: `Too heavy (${wt[i-1]} > ${w}). Must Skip. Inherit value from above: ${dp[i][w]}.`, line: 3 });
      }
    }
  }

  steps.push({ type: "done", i: -1, w: -1, dp: dp.map(r => [...r]), message: `Done. The max value for capacity ${W} is ${dp[n][W]}.`, line: 4 });
  return steps;
}

export default function Knapsack01Lesson() {
  ...
  const steps = useMemo(() => simulateKnapsack(), []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          <span className="eyebrow">Dynamic Programming • 2D Arrays</span>
          <h1>0/1 Knapsack</h1>
          <p className="description">Given a set of items with weights and values, find the subset that maximizes value without exceeding a capacity W.</p>
          <div className="complexity-tag-group">
            <div className="complexity-tag"><span className="label">Time</span><span className="value">O(N * W)</span></div>
            <div className="complexity-tag"><span className="label">Space</span><span className="value">O(N * W) or O(W)</span></div>
          </div>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>The 0/1 Knapsack is the foundation for all subset and combinatorics DP problems. You can either pick an item (1) or skip it (0).</p></article>
          <article className="guide-card"><h2>The State</h2><p>We use a 2D table `dp[i][w]`, representing the maximum value attainable using the first `i` items with a knapsack capacity of `w`.</p></article>
          <article className="guide-card"><h2>The Choices</h2><p>If the item's weight exceeds current capacity `w`, we CANNOT pick it. If it fits, we take the maximum between picking it and skipping it.</p></article>
          <article className="guide-card highlight"><h2>Space Optimization</h2><p>Because row `i` only depends on row `i-1`, we can optimize this to a 1D array of size `W`, iterating backwards to prevent reusing the same item.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the 2D DP table fill up as we decide to Pick or Skip items.</span>
              </div>
              <h2>Current Step</h2>
            ...
            <div className="canvas" style={{ display: 'flex', flexDirection: 'column', gap: '20px', padding: '20px' }}>
               ...
               <div style={{ display: 'flex', justifyContent: 'center' }}>
                 <table style={{ borderCollapse: 'collapse' }}>
                   <thead>
                     <tr>
                       <th style={{ padding: '8px', borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>Item \ Cap</th>
                       {Array.from({ length: W + 1 }).map((_, w) => (
                         <th key={w} style={{ padding: '8px', borderBottom: '1px solid var(--border)', color: step.w === w ? 'var(--blue)' : 'var(--text)' }}>{w}</th>
                       ))}
                     </tr>
                   </thead>
                   <tbody>
                     {step.dp.map((row, i) => (
                       <tr key={i}>
                         <td style={{ padding: '8px', borderRight: '1px solid var(--border)', fontWeight: 'bold', color: step.i === i ? 'var(--amber)' : 'var(--text)', fontSize: '12px' }}>
                           {i === 0 ? 'None' : `I${i} (w${wt[i-1]},v${val[i-1]})`}
                         </td>
                         {row.map((val, w) => {
                           const isActive = step.i === i && step.w === w;
                           const isSourceSkip = (step.type === "skip" || step.type === "pick_skip") && step.i === i + 1 && step.w === w;
                           const isSourcePick = step.type === "pick_skip" && step.i === i + 1 && step.w === w + (wt[step.i-1] || 0); // approx
                           ...
                           } else if (isSourceSkip || isSourcePick) {
                           ...
                           return (
                             <td key={w} style={{ 
                               padding: '8px', textAlign: 'center', width: '30px',
