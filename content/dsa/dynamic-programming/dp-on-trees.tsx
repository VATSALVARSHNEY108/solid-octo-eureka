...
function simulateTreeDP(): TreeDPStep[] {
  const steps: TreeDPStep[] = [];
  const dp = new Array(5).fill(0);
  const postOrder = [1, 2, 3, 4, 0]; // 1, 2 are leaves of 0; 3,4 leaves of 1? Let's say 3,4 children of 1.
  
  steps.push({ type: "init", node: -1, dp: [...dp], message: "Initialize DP states for all nodes in the tree.", line: 0 });

  for (const node of postOrder) {
    steps.push({ type: "visit", node, dp: [...dp], message: `Post-order traversal: Process node ${node}.`, line: 1 });
    // mock logic
    dp[node] = (node === 3 || node === 4 || node === 2) ? 1 : dp[1] + dp[2] + 1; 
    steps.push({ type: "update", node, dp: [...dp], message: `Compute state for node ${node} using its children.`, line: 2 });
  }

  steps.push({ type: "done", node: -1, dp: [...dp], message: `Done. The root node (0) contains the global answer.`, line: 3 });
  return steps;
}
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>DP on Trees</h1>
          <p className="description">Explore how to compute optimal solutions on tree data structures using Post-Order DFS traversal and state aggregation.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>Trees are natural recursive structures. A subtree is completely independent of the rest of the tree, which is perfect for DP.</p></article>
          <article className="guide-card"><h2>Post-Order Traversal</h2><p>To solve a node `u`, we must first solve all its children `v`. This strictly requires a post-order traversal (DFS).</p></article>
          <article className="guide-card"><h2>State Aggregation</h2><p>As the recursion unwinds, the parent aggregates the states returned by its children to form its own state.</p></article>
          <article className="guide-card highlight"><h2>Global Maxima</h2><p>Sometimes the global answer is not at the root. We often maintain a global `maxAns` variable that is updated during every node's processing.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the DFS traversal update states from the leaves up to the root.</span>
              </div>
              <h2>Current Step</h2>
            ...
            <div className="canvas" style={{ display: 'flex', flexDirection: 'column', gap: '20px', padding: '20px' }}>
               ...
               <div style={{ position: 'relative', width: '300px', height: '200px', margin: '0 auto' }}>
                 ...
                 <svg style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' }}>
                   <line x1="150" y1="30" x2="90" y2="90" stroke="var(--border)" strokeWidth="2" />
                   <line x1="150" y1="30" x2="210" y2="90" stroke="var(--border)" strokeWidth="2" />
                   <line x1="90" y1="90" x2="50" y2="160" stroke="var(--border)" strokeWidth="2" />
                   <line x1="90" y1="90" x2="130" y2="160" stroke="var(--border)" strokeWidth="2" />
                 </svg>
                 ...
                 {[0, 1, 2, 3, 4].map((node) => {
                   ...
                   if (node===1) {x=90; y=90;}
                   if (node===2) {x=210; y=90;}
                   if (node===3) {x=50; y=160;}
                   if (node===4) {x=130; y=160;}
                   ...
                   return (
                     <div key={node} style={{
                       ...
                       background: isActive ? 'color-mix(in srgb, var(--amber) 20%, transparent)' : 'var(--panel)',
