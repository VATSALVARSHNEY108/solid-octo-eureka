...
function simulateGridUniquePaths(): PathStep[] {
  const steps: PathStep[] = [];
  const R = 3, C = 4;
  const dp = Array.from({ length: R }, () => Array(C).fill(0));
  
  steps.push({ type: "init", r: -1, c: -1, dp: dp.map(row => [...row]), message: "Initialize a 3x4 grid for unique paths.", line: 0 });

  for (let r = 0; r < R; r++) {
    for (let c = 0; c < C; c++) {
      if (r === 0 || c === 0) {
        dp[r][c] = 1;
        steps.push({ type: "base", r, c, dp: dp.map(row => [...row]), message: `First row/column cells have only 1 unique path to them.`, line: 1 });
      } else {
        steps.push({ type: "calc", r, c, dp: dp.map(row => [...row]), message: `Calculating paths to (${r}, ${c}) by summing top and left neighbors.`, line: 2 });
        dp[r][c] = dp[r-1][c] + dp[r][c-1];
        steps.push({ type: "update", r, c, dp: dp.map(row => [...row]), message: `dp[${r}][${c}] = ${dp[r-1][c]} + ${dp[r][c-1]} = ${dp[r][c]}.`, line: 3 });
      }
    }
  }

  steps.push({ type: "done", r: -1, c: -1, dp: dp.map(row => [...row]), message: `Done. Total unique paths to bottom-right is ${dp[R-1][C-1]}.`, line: 4 });
  return steps;
}

export default function GridUniquePathsLesson() {
  const { resolvedTheme } = useTheme();
  const theme = (resolvedTheme as "light" | "dark") || "dark";

  const [stepIndex, setStepIndex] = useState(0);
  const [playing, setPlaying] = useState(false);
  const [speed, setSpeed] = useState(500); // slightly faster for 3x4
  const [isSpeechEnabled, setIsSpeechEnabled] = useState(false);
  const timerRef = useRef<number | null>(null);

  const steps = useMemo(() => simulateGridUniquePaths(), []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <p className="description">Find the number of possible unique paths from the top-left to the bottom-right of a grid, moving only down or right.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>This is the "Hello World" of 2D Grid DP. You are placed at `(0, 0)` and must reach `(m-1, n-1)` moving only down and right.</p></article>
          <article className="guide-card"><h2>The State</h2><p>Let `dp[r][c]` be the number of unique paths to reach the cell at row `r` and column `c`.</p></article>
          <article className="guide-card"><h2>The Base Cases</h2><p>Any cell in the first row can only be reached by moving right. Any cell in the first column can only be reached by moving down. Thus, they all have exactly 1 path.</p></article>
          <article className="guide-card highlight"><h2>The Transition</h2><p>For any inner cell, you could have arrived from the cell directly above it, or the cell directly to its left. Thus, `dp[r][c] = dp[r-1][c] + dp[r][c-1]`.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the number of paths accumulate dynamically.</span>
              </div>
              <h2>Current Step</h2>
               ...
               <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 60px)', gap: '8px', justifyContent: 'center' }}>
                 {step.dp.map((row, r) => 
                   row.map((val, c) => {
                     ...
                     const isSource = step.r !== -1 && step.type !== "base" && ((step.r - 1 === r && step.c === c) || (step.r === r && step.c - 1 === c));
