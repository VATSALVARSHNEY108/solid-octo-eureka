...
export default function DPOnGraphsLesson() {
  const { resolvedTheme } = useTheme();
  const theme = (resolvedTheme as "light" | "dark") || "dark";

  // This is a conceptual pattern overview page. We can use a simplified simulator showing topological DP.
  ...
  // Simplified DAG from Dag DP
  const steps = useMemo(() => {
    return [
      { msg: "Every DP problem can be modeled as a DAG.", activeNode: -1, line: 0 },
      { msg: "Nodes represent DP states.", activeNode: 0, line: 1 },
      { msg: "Edges represent transitions between states.", activeNode: 1, line: 2 },
      { msg: "We solve in Topological Order to ensure dependencies are resolved.", activeNode: 2, line: 3 },
      { msg: "Final state contains the answer.", activeNode: 3, line: 4 }
    ];
  }, []);
  ...
  const codeSnippet = [
    "// Conceptual connection between DP and Graphs",
    "// State = Node",
    "// Transition = Edge",
    "// Order of evaluation = Topological Sort",
    "// Shortest/Longest path = Min/Max DP"
  ];
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>DP on Graphs</h1>
          <p className="description">Understand the fundamental relationship between Dynamic Programming and Graph Theory.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>The Grand Unification</h2><p>Dynamic programming is just finding the shortest or longest path in a Directed Acyclic Graph (DAG).</p></article>
          <article className="guide-card"><h2>States as Nodes</h2><p>Every subproblem (e.g. `dp[i][j]`) is a node in the graph. The number of states is the number of vertices |V|.</p></article>
          <article className="guide-card"><h2>Transitions as Edges</h2><p>The recursive call (e.g. `dp[i-1][j]`) is a directed edge between nodes. The number of choices is the out-degree.</p></article>
          <article className="guide-card highlight"><h2>Acyclicity</h2><p>DP only works if there are no cycles (infinite loops in recursion). Hence, the graph of dependencies must be a DAG.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to visualize how DP maps directly to Graph traversal.</span>
              </div>
                 ...
                 {[0, 1, 2, 3].map((node, i) => (
                   <div key={node} style={{ display: 'flex', alignItems: 'center' }}>
                     <div style={{
                       ...
                       background: step.activeNode === node ? 'color-mix(in srgb, var(--blue) 20%, transparent)' : 'var(--panel)',
                       border: `2px solid ${step.activeNode === node ? 'var(--blue)' : 'var(--border)'}`,
                       fontWeight: 'bold', transition: 'all 0.3s'
                     }}>
                       dp[{node}]
                     </div>
                     {i < 3 && (
                       <div style={{ width: '40px', height: '2px', background: 'var(--border)', margin: '0 10px', position: 'relative' }}>
                         <div style={{ position: 'absolute', right: '-5px', top: '-4px', width: '0', height: '0', borderTop: '5px solid transparent', borderBottom: '5px solid transparent', borderLeft: '5px solid var(--border)' }}></div>
                       </div>
                     )}
                   </div>
                 ))}
