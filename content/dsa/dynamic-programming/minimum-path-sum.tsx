...
function simulateMinPathSum(): PathStep[] {
  ...
  steps.push({ type: "init", r: -1, c: -1, dp: dp.map(row => [...row]), message: "Initialize DP table. dp[r][c] is the minimum path sum to cell (r, c).", line: 0 });

  dp[0][0] = grid[0][0];
  steps.push({ type: "start", r: 0, c: 0, dp: dp.map(row => [...row]), message: "Start at top-left. dp[0][0] = grid[0][0].", line: 1 });

  for (let c = 1; c < C; c++) {
    dp[0][c] = dp[0][c-1] + grid[0][c];
    steps.push({ type: "row0", r: 0, c, dp: dp.map(row => [...row]), message: "First row cells can only be reached from the left.", line: 2 });
  }
  
  for (let r = 1; r < R; r++) {
    dp[r][0] = dp[r-1][0] + grid[r][0];
    steps.push({ type: "col0", r, c: 0, dp: dp.map(row => [...row]), message: "First col cells can only be reached from above.", line: 3 });
  }

  for (let r = 1; r < R; r++) {
    for (let c = 1; c < C; c++) {
      steps.push({ type: "check", r, c, dp: dp.map(row => [...row]), message: `Evaluating cell (${r}, ${c}) with cost ${grid[r][c]}.`, line: 4 });
      dp[r][c] = Math.min(dp[r-1][c], dp[r][c-1]) + grid[r][c];
      steps.push({ type: "update", r, c, dp: dp.map(row => [...row]), message: `Take the MIN of Top (${dp[r-1][c]}) and Left (${dp[r][c-1]}), plus current cell cost (${grid[r][c]}) => ${dp[r][c]}.`, line: 5 });
    }
  }

  steps.push({ type: "done", r: -1, c: -1, dp: dp.map(row => [...row]), message: `Done. The minimum path sum to the bottom-right is ${dp[R-1][C-1]}.`, line: 6 });
  return steps;
}

export default function MinimumPathSumLesson() {
  ...
  const steps = useMemo(() => simulateMinPathSum(), []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <p className="description">Find a path from top-left to bottom-right in a grid, moving only down or right, which minimizes the sum of all numbers along its path.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>This is a variation of Grid Unique Paths, but instead of counting ways, we want to optimize a cost function (the sum of values).</p></article>
          <article className="guide-card"><h2>The Logic</h2><p>Since you can only move down or right, the cell `(r, c)` can only be entered from `(r-1, c)` (top) or `(r, c-1)` (left). To minimize the total cost at `(r, c)`, you must choose the minimum of those two entry paths.</p></article>
          <article className="guide-card"><h2>Base Cases</h2><p>The top boundary can only be reached by moving right. The left boundary can only be reached by moving down. We prefill these to avoid out-of-bounds checks in the main loop.</p></article>
          <article className="guide-card highlight"><h2>Space Optimization</h2><p>Since we only look at the current row and the row above it, we can optimize space to O(N) by keeping just two 1D arrays, or mutating the original grid.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the Minimum Path Sum accumulate.</span>
              </div>
              <h2>Current Step</h2>
               ...
               <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 60px)', gap: '8px', justifyContent: 'center' }}>
                 {step.dp.map((row, r) => 
                   row.map((val, c) => {
                     ...
                     const isSource = step.r !== -1 && step.type !== "row0" && step.type !== "col0" && ((step.r - 1 === r && step.c === c) || (step.r === r && step.c - 1 === c));
