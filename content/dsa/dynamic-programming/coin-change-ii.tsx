...
function simulateCoinChangeII(coins: number[], amount: number): CoinStep[] {
  const steps: CoinStep[] = [];
  const dp = new Array(amount + 1).fill(0);
  dp[0] = 1;

  steps.push({ type: "init", amount: -1, coinIndex: -1, dp: [...dp], message: `Initialize DP array of size ${amount + 1} with 0, and dp[0] = 1 (1 way to make 0 amount).`, line: 0 });
  ...
  for (let cIdx = 0; cIdx < coins.length; cIdx++) {
    ...
    for (let i = coin; i <= amount; i++) {
      steps.push({ type: "check_amount", amount: i, coinIndex: cIdx, dp: [...dp], message: `Adding ways to make amount ${i} using coin ${coin}.`, line: 2 });
      
      const prevWays = dp[i - coin];
      if (prevWays > 0) {
        dp[i] += prevWays;
        steps.push({ type: "update", amount: i, coinIndex: cIdx, dp: [...dp], message: `dp[${i}] += dp[${i - coin}] (${prevWays}). New ways = ${dp[i]}.`, line: 3 });
      } else {
        steps.push({ type: "no_update", amount: i, coinIndex: cIdx, dp: [...dp], message: `dp[${i - coin}] is 0. No new ways to add.`, line: 3 });
      }
    }
  }

  steps.push({ type: "done", amount, coinIndex: -1, dp: [...dp], message: `Done. Total ways to make ${amount} is ${dp[amount]}.`, line: 4 });
  return steps;
}

export default function CoinChangeIILesson() {
  ...
  const steps = useMemo(() => simulateCoinChangeII(coins, amount), [coins, amount]);
  ...
  const codeSnippet = [
    "let dp = Array(amount + 1).fill(0); dp[0] = 1;",
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>Coin Change II (Number of Ways)</h1>
          <p className="description">Find the total number of combinations that make up a given amount using given coins.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>This is a combinatorics variant of the unbounded knapsack problem. Instead of minimizing, we are summing all distinct ways to form amounts.</p></article>
          <article className="guide-card"><h2>State Variable</h2><p>Let `dp[i]` be the number of ways to make amount `i`. Initially, `dp[0] = 1` (doing nothing is 1 way).</p></article>
          <article className="guide-card"><h2>Transitions</h2><p>For each coin, any amount `i` can be reached by adding the coin to amount `i - coin`. So we add `dp[i - coin]` to `dp[i]`.</p></article>
          <article className="guide-card highlight"><h2>Key Insight</h2><p>Iterating coins on the outer loop guarantees we only consider combinations (ignoring order) rather than permutations.</p></article>
        </div>
      </section>
      <section id="simulator" className="simulator">
        <div className="simulator-content">
          <div className="editor" style={{ display: 'flex', gap: '8px', padding: '0 40px', alignItems: 'center' }}>
            ...
            <button onClick={() => { setCoinsInput("1, 2, 5"); setCoins([1, 2, 5]); setAmount(5); }}>Reset</button>
          </div>
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>🖱️ Adjust the available coins and target amount.</span>
                <span>▶️ Play to watch how combinations accumulate for each amount.</span>
              </div>
              <h2>Current Step</h2>
