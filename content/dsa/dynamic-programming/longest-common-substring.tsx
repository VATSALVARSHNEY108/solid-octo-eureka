...
function simulateLCSubstring(): LCSubStep[] {
  const steps: LCSubStep[] = [];
  const s1 = "abcdf";
  const s2 = "abzdf";
  const m = s1.length;
  const n = s2.length;
  ...
  steps.push({ type: "init", i: -1, j: -1, dp: dp.map(r => [...r]), maxLen: 0, message: "Initialize 2D DP table. If characters don't match, length is 0 (must be contiguous).", line: 0 });

  let maxLen = 0;

  for (let i = 1; i <= m; i++) {
    for (let j = 1; j <= n; j++) {
      steps.push({ type: "check", i, j, dp: dp.map(r => [...r]), maxLen, message: `Comparing '${s1[i-1]}' and '${s2[j-1]}'.`, line: 1 });
      
      if (s1[i-1] === s2[j-1]) {
        dp[i][j] = 1 + dp[i-1][j-1];
        steps.push({ type: "match", i, j, dp: dp.map(r => [...r]), maxLen, message: `Match! Add 1 to the diagonal: dp[${i-1}][${j-1}] + 1 = ${dp[i][j]}.`, line: 2 });
        
        if (dp[i][j] > maxLen) {
          maxLen = dp[i][j];
          steps.push({ type: "new_max", i, j, dp: dp.map(r => [...r]), maxLen, message: `New maximum contiguous substring length found: ${maxLen}.`, line: 3 });
        }
      } else {
        dp[i][j] = 0;
        steps.push({ type: "mismatch", i, j, dp: dp.map(r => [...r]), maxLen, message: `Mismatch. Reset to 0 since substring must be contiguous.`, line: 4 });
      }
    }
  }

  steps.push({ type: "done", i: -1, j: -1, dp: dp.map(r => [...r]), maxLen, message: `Done. The Longest Common Substring length is ${maxLen}.`, line: 5 });
  return steps;
}

export default function LongestCommonSubstringLesson() {
  ...
  const steps = useMemo(() => simulateLCSubstring(), []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <p className="description">Find the length of the longest contiguous sequence of characters that appear in both strings. A stricter variation of LCS.</p>
          <div className="complexity-tag-group">
            <div className="complexity-tag"><span className="label">Time</span><span className="value">O(M * N)</span></div>
            <div className="complexity-tag"><span className="label">Space</span><span className="value">O(M * N) or O(min(M,N))</span></div>
          </div>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>Unlike Longest Common Subsequence, a Substring must be contiguous. This means the moment two characters don't match, the streak is broken.</p></article>
          <article className="guide-card"><h2>The State</h2><p>`dp[i][j]` represents the length of the longest common substring ENDING at `s1[i-1]` and `s2[j-1]`.</p></article>
          <article className="guide-card"><h2>The Reset</h2><p>If characters match, `dp[i][j] = 1 + dp[i-1][j-1]`. If they do NOT match, `dp[i][j] = 0`. We don't carry over the max from Top or Left like in LCS.</p></article>
          <article className="guide-card highlight"><h2>Global Max</h2><p>Because the longest substring could end anywhere in the strings, the final answer isn't necessarily `dp[m][n]`. We track `maxLen` dynamically during the loop.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the 2D grid build. Notice how mismatches immediately force a 0, breaking the diagonal chains.</span>
              </div>
              <h2>Current Step</h2>
              ...
              <div style={{ marginTop: '20px', padding: '16px', background: 'var(--panel2)', borderRadius: '8px', border: '2px solid var(--amber)' }}>
                <span style={{ fontSize: '12px', color: 'var(--muted)', textTransform: 'uppercase' }}>Global Max Length</span>
                <div style={{ fontSize: '24px', fontWeight: 'bold', color: 'var(--amber)' }}>{step.maxLen}</div>
              </div>
            </aside>
                     ...
                     {step.dp.map((row, i) => (
                       <tr key={i}>
                         ...
                         {row.map((val, j) => {
                           ...
                           const isDiagonalSource = step.type.startsWith("match") && step.i === i + 1 && step.j === j + 1;
                           
                           let bg = 'transparent';
                           let border = '1px solid var(--border)';
                           let color = val === 0 ? 'var(--muted)' : 'var(--green)';
                           ...
                           return (
                             <td key={j} style={{ 
                               padding: '8px', textAlign: 'center', width: '35px',
                               background: bg, border, transition: 'all 0.2s',
                               color, fontWeight: val > 0 ? 'bold' : 'normal'
                             }}>
                               {val}
                             </td>
                           );
