...
function simulateUnboundedKnapsack(): UnboundedStep[] {
  const steps: UnboundedStep[] = [];
  const W = 8;
  const wt = [2, 3, 4];
  const val = [1, 2, 5];
  ...
  for (let w = 0; w <= W; w++) {
    steps.push({ type: "outer", w, i: -1, dp: [...dp], message: `Processing capacity W = ${w}.`, line: 1 });
    
    for (let i = 0; i < wt.length; i++) {
      if (wt[i] <= w) {
        steps.push({ type: "check", w, i, dp: [...dp], message: `Item ${i} (wt: ${wt[i]}, val: ${val[i]}) fits in capacity ${w}.`, line: 2 });
        const oldVal = dp[w];
        ...
        if (dp[w] > oldVal) {
          ...
          steps.push({ type: "skip", w, i, dp: [...dp], message: `Keeping dp[${w}] = ${oldVal}. New value ${dp[w - wt[i]] + val[i]} is not greater.`, line: 4 });
        }
      }
    }
  }
...
export default function UnboundedKnapsackLesson() {
  ...
  const steps = useMemo(() => simulateUnboundedKnapsack(), []);
  ...
  const codeSnippet = [
    ...
    "// Note: loops can be swapped in Unbounded Knapsack",
    "for (let w = 0; w <= W; w++) {",
    "  for (let i = 0; i < wt.length; i++) {",
    "    if (wt[i] <= w) {",
    "      dp[w] = Math.max(dp[w], dp[w - wt[i]] + val[i]);",
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>Unbounded Knapsack</h1>
          <p className="description">Given weights and values of items, put them in a knapsack of capacity W to get the maximum value. You are allowed to use an unlimited number of instances of an item.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>Unlike 0/1 Knapsack where you can only use an item once, Unbounded Knapsack allows you to pick an item as many times as you want as long as it fits.</p></article>
          <article className="guide-card"><h2>1D DP Array</h2><p>We use a 1D array `dp` of size `W + 1`. `dp[w]` stores the maximum value achievable with a knapsack capacity of exactly `w`.</p></article>
          <article className="guide-card"><h2>The Transition</h2><p>For a capacity `w`, we try adding every item `i`. If `wt[i] &lt;= w`, the new value is `dp[w - wt[i]] + val[i]`. We take the max over all items.</p></article>
          <article className="guide-card highlight"><h2>Forward Traversal</h2><p>Unlike 1D 0/1 Knapsack which iterates the capacity backwards, we iterate forwards. This is because we *want* to be able to reuse the same item multiple times (e.g., to compute `dp[w]`, we can use the already-updated `dp[w - wt[i]]`).</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch how values are accumulated from left to right, allowing items to be used repeatedly.</span>
              </div>
              <h2>Current Step</h2>
              ...
              <label>Speed<input type="range" min="100" max="1500" value={speed} onChange={(e) => setSpeed(Number(e.target.value))} /></label>
            ...
            <div className="canvas" style={{ display: 'flex', flexDirection: 'column', gap: '20px', padding: '20px' }}>
               ...
               <div style={{ textAlign: 'center', color: 'var(--muted)', fontSize: '13px', marginBottom: '8px' }}>DP Array (Max Value at Capacity W)</div>
