...
// Rod Cutting is essentially Unbounded Knapsack
interface RodStep {
  type: string;
  len: number;
  j: number;
...
function simulateRodCutting(): RodStep[] {
  const steps: RodStep[] = [];
  const prices = [1, 5, 8, 9]; // Prices for lengths 1, 2, 3, 4
  const n = prices.length;
  const dp = new Array(n + 1).fill(0);

  steps.push({ type: "init", len: -1, j: -1, dp: [...dp], message: "Initialize DP array with 0. dp[i] will store max value for a rod of length i.", line: 0 });

  for (let len = 1; len <= n; len++) {
    steps.push({ type: "outer", len, j: -1, dp: [...dp], message: `Considering cut of length ${len} (price ${prices[len-1]}).`, line: 1 });
    
    // We iterate left to right because we have an INFINITE supply of cuts (Unbounded Knapsack)
    for (let j = len; j <= n; j++) {
      steps.push({ type: "inner", len, j, dp: [...dp], message: `Can we cut length ${len} from a rod of length ${j}? Yes.`, line: 2 });
      
      const newProfit = dp[j - len] + prices[len - 1];
      if (newProfit > dp[j]) {
        dp[j] = newProfit;
        steps.push({ type: "update", len, j, dp: [...dp], message: `New max profit for length ${j}: dp[${j - len}] + ${prices[len - 1]} = ${dp[j]}.`, line: 3 });
      } else {
        steps.push({ type: "skip", len, j, dp: [...dp], message: `Existing profit ${dp[j]} is better or equal. Skip.`, line: 4 });
      }
    }
  }

  steps.push({ type: "done", len: -1, j: -1, dp: [...dp], message: `Done. The max profit for a rod of length ${n} is ${dp[n]}.`, line: 5 });
  return steps;
}

export default function RodCuttingLesson() {
  ...
  const steps = useMemo(() => simulateRodCutting(), []);
  ...
  const codeSnippet = [
    ...
    "// Note: length is 1-indexed, so price index is length-1",
    "for (let len = 1; len <= n; len++) {",
    "  // Left-to-right traversal means we can use the same length multiple times",
    "  for (let j = len; j <= n; j++) {",
    "    dp[j] = Math.max(dp[j], dp[j - len] + prices[len - 1]);",
    "  }",
    "}",
    "return dp[n];"
  ];
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>Rod Cutting</h1>
          <p className="description">Given a rod of length n and an array of prices for all pieces of size smaller than n, find the maximum value obtained by cutting up the rod and selling the pieces.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overview</h2><p>This is the classic Unbounded Knapsack problem. You have a "knapsack" (rod) of capacity `n`, and you want to fill it with "items" (cuts) that have weight (length) and value (price).</p></article>
          <article className="guide-card"><h2>Infinite Supply</h2><p>Because you can cut multiple pieces of the same length (e.g., cut a rod of length 4 into four pieces of length 1), we iterate the inner loop from LEFT to RIGHT.</p></article>
          <article className="guide-card"><h2>The Recurrence</h2><p>`dp[j]` represents the max profit for a rod of length `j`. To update it, we see if making a cut of `len` improves it: `dp[j - len] + price[len]`.</p></article>
          <article className="guide-card highlight"><h2>1D Space Optimization</h2><p>Because we only need the answers from the current row and the left side of the current row, a single 1D array perfectly captures the state transitions.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the algorithm test different cut sizes. Left-to-right traversal allows reusing the same cut size!</span>
              </div>
              <h2>Current Step</h2>
            ...
            <div className="canvas" style={{ display: 'flex', flexDirection: 'column', gap: '20px', padding: '20px' }}>
               ...
               <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', marginBottom: '20px' }}>
                 {prices.map((val, idx) => {
                   const len = idx + 1;
                   const isActive = step.len === len;
                   
                   return (
                     <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                       <span style={{ fontSize: '10px', color: 'var(--muted)', marginBottom: '4px' }}>Len: {len}</span>
                       <div style={{
                         width: `${len * 20 + 20}px`, height: '40px', borderRadius: '4px',
               ...
               <div style={{ textAlign: 'center', color: 'var(--muted)', fontSize: '13px', marginBottom: '8px' }}>DP Array (Max Profit for Rod Length J)</div>
               <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                 {step.dp.map((val, j) => {
                   const isTarget = step.j === j;
                   const isSource = step.len > 0 && step.j > 0 && j === step.j - step.len;
                   ...
                   if (isTarget) {
                     border = 'var(--amber)';
                     if (step.type === "update") bg = 'color-mix(in srgb, var(--green) 30%, transparent)';
                   } else if (isSource) {
                     border = 'var(--blue)';
                   }
                   ...
                   return (
                     <div key={j} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                       <span style={{ fontSize: '10px', color: 'var(--muted)', marginBottom: '2px' }}>j={j}</span>
                       <div style={{
