...
function simulatePostfixToInfix(): ConvertStep[] {
  const steps: ConvertStep[] = [];
  const exp = "ab*c+";
  const stack: string[] = [];

  steps.push({ type: "init", charIndex: -1, stack: [...stack], message: `Convert Postfix to Infix: ${exp}`, line: 0 });
  ...
  for (let i = 0; i < exp.length; i++) {
    ...
    if (char.match(/[a-zA-Z0-9]/)) {
      ...
      stack.push(newExp);
      steps.push({ type: "operator", charIndex: i, stack: [...stack], message: `'${char}' is an operator. Pop two values: ${val2} and ${val1}. Combine as ${newExp} and push back.`, line: 4 });
    }
  }

  steps.push({ type: "done", charIndex: exp.length, stack: [...stack], message: `Done! Final Infix: ${stack[0]}`, line: 6 });

  return steps;
}

export default function PostfixToInfixLesson() {
  ...
  const steps = useMemo(() => simulatePostfixToInfix(), []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <p className="description">Converting a computer-friendly Postfix expression back into human-readable Infix notation requires explicitly re-introducing parentheses to guarantee correctness.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>The String Stack</h2><p>Instead of storing operators (like Infix -&gt; Postfix), our stack will now store fully formed String chunks representing valid sub-expressions.</p></article>
          <article className="guide-card highlight"><h2>The Assembly</h2><p>When an operator is encountered, we pop `val2` then `val1`. We immediately wrap them with the operator and parentheses: `(val1 operator val2)`, then push this string back to the stack.</p></article>
          <article className="guide-card"><h2>Final Output</h2><p>By the time the loop ends, the stack will contain exactly one string. That single string is your fully parenthesized Infix expression.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the String Stack assemble larger and larger parenthesized chunks.</span>
              </div>
              <h2>Current Step</h2>
