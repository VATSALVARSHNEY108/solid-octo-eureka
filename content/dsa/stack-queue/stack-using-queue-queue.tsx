...
export default function StackUsingQueueQueueLesson() {
  const { resolvedTheme } = useTheme();
  const theme = (resolvedTheme as "light" | "dark") || "dark";
  const router = useRouter();

  // Redirect to the properly named file
  useEffect(() => {
    router.replace('/dsa/stack-queue/stack-using-queue');
  }, [router]);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          <h1>Redirecting...</h1>
          <p className="description">Redirecting to Stack Using Queues.</p>
        </div>
      </section>
      <style jsx>{`
