...
function simulateRedundantBrackets(): RedundantStep[] {
  const steps: RedundantStep[] = [];
  const exp = "((a+b))";
  const stack: string[] = [];

  steps.push({ type: "init", charIndex: -1, stack: [...stack], message: `Check for Redundant Brackets in: ${exp}`, line: 0 });
  ...
  for (let i = 0; i < exp.length; i++) {
    ...
    if (char === ')' || char === '}' || char === ']') {
      ...
      steps.push({ type: "close_bracket", charIndex: i, stack: [...stack], message: `'${char}' found. Begin popping to find operators.`, line: 3 });
      ...
      if (isRedundant) {
        steps.push({ type: "found_redundant", charIndex: i, stack: [...stack], message: `Popped: "${poppedElements}". NO operators were found between the brackets! These brackets are REDUNDANT.`, line: 8 });
        return steps;
      } else {
        steps.push({ type: "valid_bracket", charIndex: i, stack: [...stack], message: `Popped: "${poppedElements}". Operators found. Brackets are useful.`, line: 10 });
      }

    } else {
      stack.push(char);
      steps.push({ type: "push", charIndex: i, stack: [...stack], message: `Push '${char}' to stack.`, line: 14 });
    }
  }

  steps.push({ type: "done", charIndex: -1, stack: [...stack], message: `Finished string. No redundant brackets found.`, line: 16 });

  return steps;
}

export default function RedundantBracketsLesson() {
  ...
  const steps = useMemo(() => simulateRedundantBrackets(), []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>Redundant Brackets</h1>
          <p className="description">Given a mathematical expression, determine if it contains any redundant brackets. `(a+b)` is useful, but `((a+b))` contains a redundant outer shell.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>The Definition</h2><p>Brackets are only useful if they enclose an operator. For example `(a+b)` encloses `+`. `(a)` encloses no operators, making it mathematically useless (redundant).</p></article>
          <article className="guide-card highlight"><h2>The Stack Strategy</h2><p>Push everything (open brackets, operators, operands) onto a stack EXCEPT closing brackets. When you see a closing bracket, start popping.</p></article>
          <article className="guide-card"><h2>The Check</h2><p>Pop until you hit the matching open bracket. If you didn't pop any operators (`+`, `-`, `*`, `/`) during that process, the brackets were redundant!</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the Stack check for operators between bracket pairs.</span>
              </div>
              <h2>Current Step</h2>
            ...
            <div className="canvas" style={{ display: 'flex', flexDirection: 'column', gap: '30px', padding: '20px', alignItems: 'center' }}>
               ...
               <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', marginTop: '20px' }}>
                 <span style={{ fontSize: '12px', color: 'var(--muted)', marginBottom: '8px' }}>Analysis Stack</span>
                 <div style={{ 
