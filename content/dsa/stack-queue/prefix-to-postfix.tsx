...
function simulatePrefixToPostfix(): ConvertStep[] {
  const steps: ConvertStep[] = [];
  const exp = "*+ab-cd"; // Prefix for (a+b)*(c-d). Postfix should be ab+cd-*
  const stack: string[] = [];

  steps.push({ type: "init", charIndex: -1, stack: [...stack], message: `Convert Prefix to Postfix: ${exp}`, line: 0 });
  ...
  for (let i = exp.length - 1; i >= 0; i--) {
    ...
    if (char.match(/[a-zA-Z0-9]/)) {
      ...
      const newExp = `${val1}${val2}${char}`;
      stack.push(newExp);
      steps.push({ type: "operator", charIndex: i, stack: [...stack], message: `'${char}' is an operator. Pop val1 (${val1}) and val2 (${val2}). Combine as Postfix '${newExp}' and push back.`, line: 4 });
    }
  }

  steps.push({ type: "done", charIndex: -1, stack: [...stack], message: `Done! Final Postfix: ${stack[0]}`, line: 6 });

  return steps;
}

export default function PrefixToPostfixLesson() {
  ...
  const steps = useMemo(() => simulatePrefixToPostfix(), []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>Prefix to Postfix Conversion</h1>
          <p className="description">To convert Prefix to Postfix, we use a String Stack and read backwards. The logic is identical to Prefix-to-Infix, but we don't add parentheses and we put the operator at the end of the combined string.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>The Setup</h2><p>Read the Prefix string Right-to-Left. Push operands as 1-character strings.</p></article>
          <article className="guide-card highlight"><h2>The Assembly</h2><p>When you hit an operator, pop `val1` then `val2`. Combine them in pure Postfix format: `val1 + val2 + operator`.</p></article>
          <article className="guide-card"><h2>No Parentheses</h2><p>Because we are outputting Postfix, we never add parentheses! The output string remains highly compressed.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the string assembly from Prefix to Postfix format.</span>
              </div>
              <h2>Current Step</h2>
