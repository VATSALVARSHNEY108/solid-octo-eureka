...
function simulateNGE(): NGEStep[] {
  const steps: NGEStep[] = [];
  const arr = [4, 12, 5, 3, 1, 2, 5, 3, 1, 2, 4, 6];
  // make it shorter for UI
  const nums = [4, 12, 5, 3, 1, 2, 6];
  const stack: number[] = [];
  const result: number[] = new Array(nums.length).fill(-1);

  steps.push({ type: "init", idx: -1, stack: [...stack], result: [...result], message: `Find Next Greater Element for [4, 12, 5, 3, 1, 2, 6]. Iterate Right-to-Left.`, line: 0 });
  ...
  for (let i = nums.length - 1; i >= 0; i--) {
    ...
    while (stack.length > 0 && stack[stack.length - 1] <= nums[i]) {
      ...
      steps.push({ 
        ...
        message: `${popped} is <= ${nums[i]}. It can't be the Next Greater for ${nums[i]} or anything to its left. Pop it!`,
        line: 2 
      });
    }
    ...
    if (stack.length > 0) {
      ...
      steps.push({ type: "empty", idx: i, stack: [...stack], result: [...result], message: `Stack is empty. No greater element exists to the right. Record -1.`, line: 5 });
    }

    stack.push(nums[i]);
    steps.push({ type: "push", idx: i, stack: [...stack], result: [...result], message: `Push ${nums[i]} to stack. It might be the Next Greater for elements further left.`, line: 7 });
  }
...
export default function NextGreaterElementLesson() {
  ...
  const steps = useMemo(() => simulateNGE(), []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>Next Greater Element (NGE)</h1>
          <p className="description">Given an array, find the first strictly greater element that is to the right of each element. If none exists, return -1. This is the classic Monotonic Stack problem.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Right-to-Left Traversal</h2><p>By traversing backwards, we process the elements that appear on the "right side" first, storing them in the stack for future leftward elements to query.</p></article>
          <article className="guide-card highlight"><h2>The Pops</h2><p>If the stack's top element is `&lt;= nums[i]`, we pop it. Why? Because `nums[i]` is both LARGER and MORE TO THE LEFT. It completely "blocks" the popped element from ever being the NGE for anything further left!</p></article>
          <article className="guide-card"><h2>The Pushes</h2><p>After finding the NGE from the stack, we push `nums[i]` onto the stack. The stack naturally maintains a Monotonic Decreasing order.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the Right-to-Left traversal. Notice how small elements are popped and forgotten forever because they are "shadowed" by taller bars.</span>
              </div>
              <h2>Current Step</h2>
            ...
            <div className="canvas" style={{ display: 'flex', flexDirection: 'column', gap: '30px', padding: '20px', alignItems: 'center' }}>
               ...
               <span style={{ fontSize: '11px', color: 'var(--muted)' }}>Top row: Array | Bottom row: NGE Result</span>
