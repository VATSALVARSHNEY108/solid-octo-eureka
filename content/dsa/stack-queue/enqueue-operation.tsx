...
function simulateEnqueue(): EnqueueStep[] {
  const steps: EnqueueStep[] = [];
  const size = 5;
  const q: (number | null)[] = [10, 20, null, null, null];
  let front = 0;
  let rear = 1;
  ...
  function enqueue(val: number, lineIdx: number) {
    if (rear === size - 1) {
      steps.push({ type: "error", front, rear, queue: [...q], message: `Queue is FULL! Cannot enqueue ${val} (Overflow).`, line: 1 });
      return;
    }

    if (front === -1) {
      front = 0;
      steps.push({ type: "set_front", front, rear, queue: [...q], message: `Queue was empty. Set front to 0.`, line: 2 });
    }

    rear++;
    steps.push({ type: "increment", front, rear, queue: [...q], message: `Increment Rear to index ${rear}.`, line: 3 });

    q[rear] = val;
    steps.push({ type: "insert", front, rear, queue: [...q], message: `Insert ${val} at index ${rear} (Rear).`, line: 4 });
  }

  enqueue(30, 1);
  enqueue(40, 1);
  enqueue(50, 1);
  enqueue(60, 1); // Triggers overflow

  return steps;
}

export default function EnqueueOperationLesson() {
  ...
  const steps = useMemo(() => simulateEnqueue(), []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>Enqueue Operation</h1>
          <p className="description">Enqueue is the process of adding an element to a Queue. In a FIFO structure, additions always happen at the 'Rear' (or Back).</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Overflow Check</h2><p>Before adding, we check if the array is full (`rear == size - 1`). If it is, trying to enqueue causes an Overflow.</p></article>
          <article className="guide-card highlight"><h2>The Rear Pointer</h2><p>We first increment the `rear` pointer to the next available empty slot, and then insert the new value at that index.</p></article>
          <article className="guide-card"><h2>First Element Special Case</h2><p>If the queue was completely empty (`front == -1`), adding the first element means we must also set `front = 0` so we can later dequeue it.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch how Enqueue shifts the Rear pointer to the right, appending new elements.</span>
              </div>
              <h2>Current Step</h2>
            ...
            <div className="canvas" style={{ display: 'flex', flexDirection: 'column', gap: '20px', padding: '20px', alignItems: 'center' }}>
               ...
               <div style={{ display: 'flex', gap: '10px', marginTop: '40px' }}>
                 {step.queue.map((val, idx) => {
                   ...
                   if (step.type === "error" && idx === step.rear) borderColor = 'var(--red)';
