...
function simulateEvalPrefix(): EvalPrefixStep[] {
  const steps: EvalPrefixStep[] = [];
  const exp = ["-", "+", "2", "*", "3", "1", "9"]; // - + 2 * 3 1 9 == 2 + (3 * 1) - 9 = -4
  const stack: number[] = [];

  steps.push({ type: "init", charIndex: -1, stack: [...stack], message: `Evaluating Prefix Expression: - + 2 * 3 1 9`, line: 0 });

  // Read right-to-left
  for (let i = exp.length - 1; i >= 0; i--) {
    const token = exp[i];
    steps.push({ type: "read", charIndex: i, stack: [...stack], message: `Reading token (right-to-left): '${token}'.`, line: 1 });
...
export default function EvaluatePrefixLesson() {
  ...
  const steps = useMemo(() => simulateEvalPrefix(), []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>Evaluate Prefix Expression</h1>
          <p className="description">Prefix expressions (Polish Notation) are evaluated using a Stack similarly to Postfix, but we process the expression backwards (Right-to-Left).</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>Right-to-Left</h2><p>Instead of reading the expression left-to-right (like Postfix), we read a Prefix expression strictly from right-to-left.</p></article>
          <article className="guide-card highlight"><h2>Pop Order</h2><p>Because we process backwards, when an operator is found, the <i>first</i> pop is `val1` (left operand) and the <i>second</i> pop is `val2` (right operand). This is the exact opposite of Postfix!</p></article>
          <article className="guide-card"><h2>Why Prefix?</h2><p>LISP and similar languages use Polish Notation. It removes the need for order-of-operations logic or parentheses entirely, just like Postfix.</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the right-to-left evaluation. Note how the popped values form the operation intuitively.</span>
              </div>
              <h2>Current Step</h2>
