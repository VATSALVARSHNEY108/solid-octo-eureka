...
function simulatePSE(): PSEStep[] {
  ...
  steps.push({ type: "init", idx: -1, stack: [...stack], result: [...result], message: `Find Previous Smaller Element for [4, 8, 5, 2, 25]. Iterate Left-to-Right.`, line: 0 });
  ...
  for (let i = 0; i < n; i++) {
    ...
    while (stack.length > 0 && stack[stack.length - 1] >= nums[i]) {
      ...
      steps.push({ 
        ...
        message: `${popped} is >= ${nums[i]}. It can't be the Previous Smaller for ${nums[i]}. Pop it!`,
        line: 2 
      });
    }
    ...
    if (stack.length > 0) {
      ...
      steps.push({ type: "empty", idx: i, stack: [...stack], result: [...result], message: `Stack is empty. No smaller element exists to the left. Record -1.`, line: 5 });
    }
...
export default function PreviousSmallerElementLesson() {
  ...
  const steps = useMemo(() => simulatePSE(), []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>Previous Smaller Element (PSE)</h1>
          <p className="description">Given an array, find the first strictly smaller element to the left of each element. This pairs with NSE to form the basis of advanced algorithms.</p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card"><h2>The Perfect Pair</h2><p>Previous Smaller Element (PSE) and Next Smaller Element (NSE) are frequently used together. By knowing the boundary on the left (PSE) and the right (NSE), you can easily calculate the span or area over which an element is the absolute minimum.</p></article>
          <article className="guide-card highlight"><h2>Increasing Stack</h2><p>Just like NSE, PSE requires a Monotonic Increasing Stack. We pop elements that are `&gt;= nums[i]`. The only difference is we traverse from Left to Right.</p></article>
          <article className="guide-card"><h2>Why it matters</h2><p>Used heavily in "Largest Rectangle in Histogram" and "Sum of Subarray Minimums".</p></article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              <div className="gesture-hint">
                <span>▶️ Play to watch the Left-to-Right traversal using an Increasing Stack.</span>
              </div>
              <h2>Current Step</h2>
            ...
            <div className="canvas" style={{ display: 'flex', flexDirection: 'column', gap: '30px', padding: '20px', alignItems: 'center' }}>
               ...
               <span style={{ fontSize: '11px', color: 'var(--muted)' }}>Top row: Array | Bottom row: PSE Result</span>
