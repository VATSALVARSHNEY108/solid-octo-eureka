...
export default function NaivePatternMatchingLab() {
  ...
  const steps = useMemo(() => {
    ...
    sArr.push({ message: "Applying search logic...", i: 1, state: "Active" });
  ...
  return (
    <div style={{width: "100vw", height:"calc(100vh - 124px)", background: COLORS.bg, color: COLORS.textWhite, fontFamily: "'JetBrains Mono', monospace", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <TheorySection 
        ...
        spaceComplexity="O(M)"
        keyPoints={['KMP uses a prefix function.', 'Z-algorithm uses a Z-array.', 'Aho-Corasick handles multiple patterns.']}
      />
      <div style={{ height: 48, background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, display: "flex", alignItems: "center", padding: "0 16px", gap: 12 }}>
        <div style={{fontSize: 14, fontWeight: 800 }}>NAIVE_PATTERN_MATCHING</div>
