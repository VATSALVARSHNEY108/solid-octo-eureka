...
export default function LongestCommonSubsequenceStringsLab() {
  ...
  const steps = useMemo(() => {
    ...
    sArr.push({ message: "Applying dp logic...", i: 1, state: "Active" });
  ...
  return (
    <div style={{width: "100vw", height:"calc(100vh - 124px)", background: COLORS.bg, color: COLORS.textWhite, fontFamily: "'JetBrains Mono', monospace", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <TheorySection 
        title="Longest Common Subsequence Strings"
        ...
        keyPoints={['Identify the state (i, j).', 'Define the base cases.', 'Formulate the transition relation.']}
      />
      <div style={{ height: 48, background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, display: "flex", alignItems: "center", padding: "0 16px", gap: 12 }}>
        <div style={{fontSize: 14, fontWeight: 800 }}>LONGEST_COMMON_SUBSEQUENCE_STRINGS</div>
