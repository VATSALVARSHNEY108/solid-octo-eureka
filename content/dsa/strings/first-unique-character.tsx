...
export default function FirstUniqueCharacterLab() {
  ...
  const steps = useMemo(() => {
    ...
    sArr.push({ message: "Applying hashing logic...", i: 1, state: "Active" });
  ...
  return (
    <div style={{width: "100vw", height:"calc(100vh - 124px)", background: COLORS.bg, color: COLORS.textWhite, fontFamily: "'JetBrains Mono', monospace", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <TheorySection 
        ...
        keyPoints={['Frequency maps are common for anagrams.', 'Rolling hashes enable fast pattern matching.', 'Watch for hash collisions.']}
      />
      <div style={{ height: 48, background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, display: "flex", alignItems: "center", padding: "0 16px", gap: 12 }}>
        <div style={{fontSize: 14, fontWeight: 800 }}>FIRST_UNIQUE_CHARACTER</div>
