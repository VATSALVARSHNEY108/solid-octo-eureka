...
export default function SlidingWindowStringsLab() {
  ...
  const steps = useMemo(() => {
    ...
    sArr.push({ message: "Applying sliding-window logic...", i: 1, state: "Active" });
  ...
  return (
    <div style={{width: "100vw", height:"calc(100vh - 124px)", background: COLORS.bg, color: COLORS.textWhite, fontFamily: "'JetBrains Mono', monospace", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <TheorySection 
        ...
        keyPoints={['Use a frequency map to track characters.', 'Expand the right pointer until property breaks.', 'Contract the left pointer to restore property.']}
      />
      <div style={{ height: 48, background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, display: "flex", alignItems: "center", padding: "0 16px", gap: 12 }}>
        <div style={{fontSize: 14, fontWeight: 800 }}>SLIDING_WINDOW_STRINGS</div>
