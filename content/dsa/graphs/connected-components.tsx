...
import { Play, Pause, RotateCcw, ChevronLeft, ChevronRight, Info, MousePointer2, Network } from "lucide-react";
...
function generateComponentSteps(nodes: GraphNode[], edges: GraphEdge[]): ComponentStep[] {
  const steps: ComponentStep[] = [];
  const visited = new Set<string>();
  const components: string[][] = [];

  steps.push({ type: "init", visited: [], components: [], currentQueue: [], message: "Initial state: All nodes unvisited." });
  ...
  for (const node of nodes) {
    if (!visited.has(node.id)) {
      ...
      visited.add(node.id);
      steps.push({ type: "start", visited: Array.from(visited), components: [...components], currentQueue: [...queue], message: `Found unvisited node ${node.id}. New component.` });

      while (queue.length > 0) {
        const u = queue.shift()!;
        currentComponent.push(u);
        steps.push({ type: "visit", u, visited: Array.from(visited), components: [...components, [...currentComponent]], currentQueue: [...queue], message: `Visiting ${u}.` });
        const neighbors = edges.filter(e => e.from === u || e.to === u).map(e => e.from === u ? e.to : e.from);
        for (const v of neighbors) {
          if (!visited.has(v)) {
            visited.add(v); queue.push(v);
            steps.push({ type: "enqueue", u: v, visited: Array.from(visited), components: [...components, [...currentComponent]], currentQueue: [...queue], message: `Found neighbor ${v}. Enqueued.` });
          }
        }
      }
      components.push(currentComponent);
      steps.push({ type: "finish", visited: Array.from(visited), components: [...components], currentQueue: [], message: `Finished Component ${components.length}.` });
    }
  }
  steps.push({ type: "done", visited: Array.from(visited), components: [...components], currentQueue: [], message: `Done. Found ${components.length} components.` });
  return steps;
}

export default function ConnectedSimulation() {
  ...
  const [compPos, setCompPos] = useState({ x: 20, y: 20 });
  ...
  const steps = useMemo(() => generateComponentSteps(graph.nodes, graph.edges), [graph]);
  ...
  const handleMove = (e: React.PointerEvent) => {
    ...
    } else if (activeDragPanel === "comp") {
      setCompPos({ x: pt.x - dragOffset.x, y: pt.y - dragOffset.y });
  ...
  const COMPONENT_COLORS = ["#4f7ef8", "#35c486", "#f5a623", "#ef4444", "#a855f7"];
  ...
  return (
    <main className="page" data-theme={theme}>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card">
            <h2>Isolation</h2>
            <p>A connected component is a maximal subgraph where every pair of nodes has a path between them.</p>
          </article>
          <article className="guide-card">
            <h2>BFS Approach</h2>
            <p>Pick an unvisited node, run BFS to find all reachable nodes, mark them as a component, and repeat.</p>
          </article>
          <article className="guide-card highlight">
            <h2>Partitioning</h2>
            <p>The algorithm effectively divides the graph into independent groups. No edges exist between nodes of different components.</p>
          </article>
          <article className="guide-card">
            <h2>Applications</h2>
            <p>Used in image segmentation, social network analysis, and understanding the vulnerability of communication grids.</p>
          </article>
        </div>
      </section>
          ...
          <div className="workspace">
            ...
            <svg ref={svgRef} viewBox="0 0 680 480" onPointerDown={handleCanvasClick} onPointerMove={handleMove} onPointerUp={() => { setDragging(null); setActiveDragPanel(null); }}>
              {graph.edges.map(edge => {
                ...
                const compIdx = step.components.findIndex(c => c.includes(edge.from) && c.includes(edge.to));
                ...
                return (
                  <g key={edge.id} onPointerDown={(e) => { e.stopPropagation(); setSelected({ type: "edge", id: edge.id }); }}>
                    <line x1={from.x} y1={from.y} x2={to.x} y2={to.y} className={`edge ${compIdx !== -1 ? 'active' : ''} ${isSelected ? 'selected' : ''}`} style={compIdx !== -1 ? { stroke: COMPONENT_COLORS[compIdx % 5] } : {}} />
              ...
              {graph.nodes.map(node => {
                ...
                const current = step.u === node.id;
                const queued = step.currentQueue.includes(node.id);
                const compIdx = step.components.findIndex(c => c.includes(node.id));
                const color = compIdx !== -1 ? COMPONENT_COLORS[compIdx % 5] : 'var(--border)';
                const isSelected = selected?.type === "node" && selected.id === node.id;
                return (
                  <g key={node.id} className={`node ${visited ? 'visited' : ''} ${current ? 'current' : ''} ${queued ? 'queued' : ''} ${isSelected ? 'selected' : ''}`} onPointerDown={(e) => handleNodePointerDown(e, node)} onContextMenu={(e) => handleContextMenu(e, { type: "node", id: node.id })}>
                    <circle cx={node.x} cy={node.y} r="22" style={{ stroke: color }} />
                    <text x={node.x} y={node.y + 5}>{node.id}</text>
                  </g>
                );
              })}

              <foreignObject x={compPos.x} y={compPos.y} width="160" height="250" className="movable-panel">
                <div className="panel-container" data-theme={theme}>
                  <div className="panel-header" onPointerDown={(e) => startPanelDrag(e, "comp", compPos)}>
                    <span>Components</span>
                    <span className="drag-handle">⠿</span>
                  </div>
                  <div className="panel-content comp-viz">
                    {step.components.map((c, i) => (
                      <div key={i} className="comp-item" style={{ color: COMPONENT_COLORS[i % 5], background: `${COMPONENT_COLORS[i % 5]}15` }}>
                        #{i+1}: {c.join(", ")}
                      </div>
                    ))}
                    {step.components.length === 0 && <span className="empty-msg">Waiting...</span>}
                  </div>
                </div>
              </foreignObject>
              ...
              <foreignObject x={queuePos.x} y={queuePos.y} width="160" height="250" className="movable-panel">
                <div className="panel-container" data-theme={theme}>
                  ...
                  <div className="panel-content queue-viz">
                    {step.currentQueue.map((id, i) => (
                      <div key={i} className="queue-item">{id}</div>
                    ))}
                    {step.currentQueue.length === 0 && <span className="empty-msg">Empty</span>}
                  </div>
                </div>
              </foreignObject>
            </svg>
      ...
      <style jsx>{`
        ...
        .node.current circle { fill: var(--blue) !important; stroke: var(--blue) !important; filter: drop-shadow(0 0 10px var(--blue)); }
        .node.queued circle { stroke: var(--amber) !important; stroke-width: 5; stroke-dasharray: 4; }
        ...
        .comp-item { padding: 6px 10px; border-radius: 6px; font-size: 11px; margin-bottom: 4px; font-weight: 700; }
