...
function generateTarjanSteps(nodes: GraphNode[], edges: GraphEdge[]): TarjanStep[] {
  ...
  function dfs(u: string) {
    ...
    stack.push(u);
    onStack.add(u);
    steps.push({ type: "visit", discovery: { ...discovery }, low: { ...low }, stack: [...stack], sccs: [...sccs], u, message: `Visiting ${u}. Disc=${timer}, Low=${timer}.`, line: 1 });
    
    const neighbors = edges.filter(e => e.from === u).map(e => e.to);
    for (const v of neighbors) {
      steps.push({ type: "check", discovery: { ...discovery }, low: { ...low }, stack: [...stack], sccs: [...sccs], u, v, message: `Checking neighbor ${v} of ${u}.`, line: 3 });
      if (discovery[v] === undefined) {
        dfs(v);
        low[u] = Math.min(low[u], low[v]);
        steps.push({ type: "return", discovery: { ...discovery }, low: { ...low }, stack: [...stack], sccs: [...sccs], u, v, message: `Backtrack to ${u}. Low[${u}] updated to ${low[u]}.`, line: 6 });
      } else if (onStack.has(v)) {
        low[u] = Math.min(low[u], discovery[v]);
        steps.push({ type: "back_edge", discovery: { ...discovery }, low: { ...low }, stack: [...stack], sccs: [...sccs], u, v, message: `Back-edge ${u}→${v}. Low[${u}] updated to ${low[u]}.`, line: 8 });
      }
    }
    
    steps.push({ type: "root_check", discovery: { ...discovery }, low: { ...low }, stack: [...stack], sccs: [...sccs], u, message: `Checking if ${u} is SCC root (Low=${low[u]}, Disc=${discovery[u]}).`, line: 9 });
    if (low[u] === discovery[u]) {
      ...
      sccs.push(component);
      steps.push({ type: "scc_found", discovery: { ...discovery }, low: { ...low }, stack: [...stack], sccs: [...sccs], u, message: `SCC Root found at ${u}. Component: {${component.join(", ")}}`, line: 9 });
    }
  }
  nodes.forEach(n => { if (discovery[n.id] === undefined) dfs(n.id); });
  steps.push({ type: "done", discovery: { ...discovery }, low: { ...low }, stack: [], sccs: [...sccs], message: "Tarjan's SCC identification complete." });
  return steps;
}

export default function TarjanSimulation() {
  ...
  const steps = useMemo(() => generateTarjanSteps(graph.nodes, graph.edges), [graph]);
  ...
  return (
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <p className="description">
            A linear-time, single-pass DFS algorithm that identifies <strong>Strongly Connected Components</strong>. 
            It maintains low-link values to detect the roots of cyclic components.
          </p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card">
            <h2>Single Pass</h2>
            <p>Unlike Kosaraju's, Tarjan's identifies all SCCs in a single DFS traversal, making it slightly more efficient in practice.</p>
          </article>
          <article className="guide-card">
            <h2>Low-Link Values</h2>
            <p>Each node tracks the smallest discovery time reachable from it. A node is an SCC root if its discovery time equals its low-link value.</p>
          </article>
          <article className="guide-card highlight">
            <h2>Stack Logic</h2>
            <p>Nodes are kept on a stack until their SCC is fully explored. When a root is identified, the stack is popped to extract the component.</p>
          </article>
          <article className="guide-card">
            <h2>Cyclic Dependencies</h2>
            <p>Widely used in compiler optimization and dependency analysis to find circular references and interconnected modules.</p>
          </article>
        </div>
      </section>
          ...
          <div className="workspace">
            ...
            <svg ref={svgRef} viewBox="0 0 680 480" onPointerDown={handleCanvasClick} onPointerMove={handleMove} onPointerUp={() => { setDragging(null); setActiveDragPanel(null); }}>
              ...
              {graph.edges.map(edge => {
                ...
                const active = (step.u === edge.from && step.v === edge.to);
              ...
              {graph.nodes.map(node => {
                ...
                const inStack = step.stack.includes(node.id); const isRoot = disc === low && disc !== undefined;
                const active = step.u === node.id;
                const isSelected = selected?.type === "node" && selected.id === node.id;
                return (
                  <g key={node.id} className={`node ${disc !== undefined ? 'visited' : ''} ${inStack ? 'on-stack' : ''} ${active ? 'active' : ''} ${isSelected ? 'selected' : ''}`} onPointerDown={(e) => handleNodePointerDown(e, node)} onContextMenu={(e) => handleContextMenu(e, { type: "node", id: node.id })}>
              ...
              <foreignObject x={codePos.x} y={codePos.y} width="220" height="220" className="movable-panel">
      ...
      <style jsx>{`
        ...
        .node.on-stack circle { stroke: var(--amber); stroke-width: 4; }
        ...
        .metrics-label { font-size: 9px; fill: var(--muted); font-weight: 700; text-anchor: middle; font-family: monospace; pointer-events: none; }
