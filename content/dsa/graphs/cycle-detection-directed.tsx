...
import { Play, Pause, RotateCcw, ChevronLeft, ChevronRight, Info, MousePointer2, Layers } from "lucide-react";
...
function generateCycleSteps(nodes: GraphNode[], edges: GraphEdge[]): CycleStep[] {
  ...
  function dfs(u: string): boolean {
    ...
    steps.push({ type: "visit", u, visited: Array.from(visited), recursionStack: Array.from(recStack), message: `Visiting ${u}. Push to recursion stack.`, hasCycle: false });
    const neighbors = edges.filter(e => e.from === u).map(e => e.to);
    for (const v of neighbors) {
      if (!visited.has(v)) { if (dfs(v)) return true; }
      else if (recStack.has(v)) {
        steps.push({ type: "cycle", u, v, visited: Array.from(visited), recursionStack: Array.from(recStack), message: `Back-edge detected from ${u} to ${v}. Cycle identified!`, hasCycle: true });
        return true;
      }
    }
    recStack.delete(u);
    steps.push({ type: "backtrack", u, visited: Array.from(visited), recursionStack: Array.from(recStack), message: `Popping ${u} from stack.`, hasCycle: false });
    return false;
  }
  for (const node of nodes) if (!visited.has(node.id)) if (dfs(node.id)) break;
  if (!steps.some(s => s.hasCycle)) steps.push({ type: "done", visited: Array.from(visited), recursionStack: [], message: "No cycles detected.", hasCycle: false });
  return steps;
}

export default function DirectedCycleSimulation() {
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          ...
          <h1>Cycle Detection (Directed)</h1>
          <p className="description">
            Cycles in directed graphs are detected by finding <strong>back-edges</strong> during DFS. A back-edge points from a node to an ancestor currently in the recursion stack.
          </p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card">
            ...
            <p>We keep track of nodes currently in the DFS path. If we visit a node already in the stack, a cycle exists.</p>
          </article>
          <article className="guide-card">
            <h2>Back-Edge</h2>
            <p>A back-edge is an edge from a node to one of its ancestors in the DFS tree. This is the definitive indicator of a directed cycle.</p>
          </article>
          <article className="guide-card highlight">
            <h2>DFS Traversal</h2>
            <p>Every node and edge is explored once. If the DFS completes without finding any back-edges, the graph is a Directed Acyclic Graph (DAG).</p>
          </article>
          <article className="guide-card">
            <h2>State Marking</h2>
            <p>Nodes can be in three states: unvisited, currently visiting (in stack), or fully visited (processed).</p>
          </article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              ...
              <div className="status-panel">
                ...
                <div className={`status-pill ${step.hasCycle ? 'failure' : 'success'}`}>
                  {step.hasCycle ? "Cycle Detected" : "No Cycle Identified"}
                </div>
            ...
            <svg ref={svgRef} viewBox="0 0 680 480" onPointerDown={handleCanvasClick} onPointerMove={handleMove} onPointerUp={() => { setDragging(null); setActiveDragPanel(null); }}>
              ...
              {graph.edges.map(edge => {
                ...
                const isCycle = step.hasCycle && step.u === edge.from && step.v === edge.to;
                ...
                return (
                  <g key={edge.id} onPointerDown={(e) => { e.stopPropagation(); setSelected({ type: "edge", id: edge.id }); }}>
                    <line x1={from.x} y1={from.y} x2={to.x} y2={to.y} className={`edge ${isCycle ? 'cycle' : ''} ${isSelected ? 'selected' : ''}`} markerEnd="url(#arrow)" />
              ...
              {graph.nodes.map(node => {
                ...
                const inStack = step.recursionStack.includes(node.id);
                const active = step.u === node.id;
                const isSelected = selected?.type === "node" && selected.id === node.id;
                return (
                  <g key={node.id} className={`node ${visited ? 'visited' : ''} ${inStack ? 'in-stack' : ''} ${active ? 'active' : ''} ${isSelected ? 'selected' : ''}`} onPointerDown={(e) => handleNodePointerDown(e, node)} onContextMenu={(e) => handleContextMenu(e, { type: "node", id: node.id })}>
              ...
              <foreignObject x={stackPos.x} y={stackPos.y} width="160" height="300" className="movable-panel">
                <div className="panel-container" data-theme={theme}>
                  ...
                  <div className="panel-content stack-viz">
                    {step.recursionStack.map((id, i) => (
                      <div key={i} className="stack-item">Node {id}</div>
                    ))}
                    {step.recursionStack.length === 0 && <span className="empty-msg">Empty</span>}
                  </div>
                </div>
              </foreignObject>
            </svg>
      ...
      <style jsx>{`
        ...
        .node.in-stack circle { stroke: var(--blue); stroke-width: 5; fill: color-mix(in srgb, var(--blue) 10%, var(--panel)); }
        ...
        .stack-item { padding: 6px 10px; background: var(--panel2); color: var(--blue); border-radius: 6px; font-size: 11px; margin-bottom: 2px; font-weight: 700; text-align: center; }
