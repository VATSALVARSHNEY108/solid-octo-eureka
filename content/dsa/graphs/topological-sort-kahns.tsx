...
function generateKahnSteps(nodes: GraphNode[], edges: GraphEdge[]): KahnStep[] {
  ...
  steps.push({ type: "init", indegree: { ...indegree }, queue: [], sorted: [], message: "Initialize in-degrees for all nodes." });

  nodes.forEach(n => { if (indegree[n.id] === 0) queue.push(n.id); });
  steps.push({ type: "enqueue_initial", indegree: { ...indegree }, queue: [...queue], sorted: [], message: "Enqueue nodes with 0 in-degree." });

  while (queue.length > 0) {
    const u = queue.shift()!;
    sorted.push(u);
    steps.push({ type: "process", u, indegree: { ...indegree }, queue: [...queue], sorted: [...sorted], message: `Processing ${u}. Add to topological order.` });
    const neighbors = edges.filter(e => e.from === u).map(e => e.to);
    for (const v of neighbors) {
      indegree[v]--;
      steps.push({ type: "update", u, indegree: { ...indegree }, queue: [...queue], sorted: [...sorted], message: `Decreased in-degree of neighbor ${v}.` });
      if (indegree[v] === 0) {
        queue.push(v);
        steps.push({ type: "enqueue", u: v, indegree: { ...indegree }, queue: [...queue], sorted: [...sorted], message: `In-degree of ${v} is 0. Enqueuing.` });
      }
    }
  }
  const hasCycle = sorted.length !== nodes.length;
  steps.push({ type: "done", indegree: { ...indegree }, queue: [], sorted: [...sorted], message: hasCycle ? "Cycle detected! Incomplete sort." : "Topological Sort Complete." });
  return steps;
}

export default function KahnSimulation() {
  ...
  const steps = useMemo(() => generateKahnSteps(graph.nodes, graph.edges), [graph]);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          <span className="eyebrow">Graph Algorithms • BFS Based</span>
          <h1>Topological Sort (Kahn's)</h1>
          <p className="description">
            Kahn's algorithm uses a <strong>BFS strategy</strong> to find a topological ordering. 
            It works by maintaining the in-degree of each node and iteratively processing those with zero dependencies.
          </p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card">
            <h2>In-Degree</h2>
            <p>The number of incoming edges to a node. A node with 0 in-degree is a "source" and can be processed immediately.</p>
          </article>
          <article className="guide-card">
            <h2>Dependency</h2>
            <p>When a node is processed, it's "removed" from the graph, reducing the in-degree of all its neighbors.</p>
          </article>
          <article className="guide-card highlight">
            <h2>The Queue</h2>
            <p>A queue stores all nodes whose in-degree has become 0, ensuring we process them in a valid topological order.</p>
          </article>
          <article className="guide-card">
            <h2>Cycle Detector</h2>
            <p>If we can't process all nodes because none have 0 in-degree, the remaining nodes must form a cycle.</p>
          </article>
        </div>
      </section>
          ...
          <div className="workspace">
            ...
            <svg ref={svgRef} viewBox="0 0 680 480" onPointerDown={handleCanvasClick} onPointerMove={handleMove} onPointerUp={() => { setDragging(null); setActiveDragPanel(null); }}>
              ...
              {graph.edges.map(edge => {
                ...
                const processed = step.sorted.includes(edge.from);
                ...
                return (
                  <g key={edge.id} onPointerDown={(e) => { e.stopPropagation(); setSelected({ type: "edge", id: edge.id }); }}>
                    <line x1={from.x} y1={from.y} x2={to.x} y2={to.y} className={`edge ${processed ? 'processed' : ''} ${isSelected ? 'selected' : ''}`} markerEnd="url(#arrow)" />
                    <line x1={from.x} y1={from.y} x2={to.x} y2={to.y} className="edge-hit" onContextMenu={(e) => handleContextMenu(e, { type: "edge", id: edge.id })} />
                  </g>
                );
              })}
              {graph.nodes.map(node => {
                const indegree = step.indegree[node.id];
                const processed = step.sorted.includes(node.id);
                const queued = step.queue.includes(node.id);
                const active = step.u === node.id;
                const isSelected = selected?.type === "node" && selected.id === node.id;
                return (
                  <g key={node.id} className={`node ${processed ? 'processed' : ''} ${queued ? 'queued' : ''} ${active ? 'active' : ''} ${isSelected ? 'selected' : ''}`} onPointerDown={(e) => handleNodePointerDown(e, node)} onContextMenu={(e) => handleContextMenu(e, { type: "node", id: node.id })}>
                    <circle cx={node.x} cy={node.y} r="24" />
                    <text x={node.x} y={node.y + 5}>{node.id}</text>
                    {!processed && <text x={node.x} y={node.y + 36} className="indegree-label">In: {indegree}</text>}
                  </g>
                );
              })}
                ...
                <div className="panel-container" data-theme={theme}>
                  ...
                  <div className="panel-content queue-viz">
                    {step.queue.map((id, idx) => (
                      <div key={idx} className="queue-item">{id}</div>
                    ))}
      ...
      <style jsx>{`
        ...
        .edge.processed { opacity: 0.15; stroke-dasharray: 4; }
        ...
        .node.processed circle { stroke: var(--green); fill: color-mix(in srgb, var(--green) 10%, var(--panel)); }
        .node.queued circle { stroke: var(--amber); stroke-width: 5; stroke-dasharray: 6 3; }
        ...
        .indegree-label { font-size: 9px; fill: var(--muted); font-weight: 700; text-anchor: middle; font-family: monospace; }
        ...
        .queue-item { padding: 4px 8px; background: var(--panel2); color: var(--amber); border-radius: 6px; font-size: 11px; margin-bottom: 2px; font-weight: 700; text-align: center; }
