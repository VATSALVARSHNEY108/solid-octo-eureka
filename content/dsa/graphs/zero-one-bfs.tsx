...
const INITIAL_EDGES: GraphEdge[] = [
  { id: "e1", from: "0", to: "1", weight: 0 },
  { id: "e2", from: "0", to: "2", weight: 1 },
  { id: "e3", from: "1", to: "3", weight: 1 },
  { id: "e4", from: "2", to: "3", weight: 0 },
  { id: "e5", from: "3", to: "4", weight: 1 },
  { id: "e6", from: "4", to: "5", weight: 0 },
...
function generate01Steps(nodes: GraphNode[], edges: GraphEdge[]): BFS01Step[] {
  ...
  dist[startNode] = 0;
  const deque: string[] = [startNode];

  steps.push({
    dist: { ...dist },
    deque: [...deque],
    message: `Initialize: Set distance of source node ${startNode} to 0. We'll use a Deque for efficient 0-1 exploration.`
  });

  const adj: Record<string, { to: string; weight: number; id: string }[]> = {};
  ...
  while (deque.length > 0) {
    ...
    steps.push({
      ...
      deque: [...deque],
      u,
      message: `Extracting ${u} from the front of the Deque.`
    });
      ...
      if (dist[u] + w < dist[v]) {
        ...
        if (w === 0) {
          ...
          steps.push({
            ...
            deque: [...deque],
            u, v, weight: w,
            message: `Edge ${u}↔${v} has weight 0. Optimal distance found. Unshifting ${v} to FRONT.`
          });
        } else {
          deque.push(v);
          steps.push({
            ...
            deque: [...deque],
            u, v, weight: w,
            message: `Edge ${u}↔${v} has weight 1. Adding ${v} to the BACK of the Deque.`
          });
        }
      }
    }
  ...
  steps.push({
    dist: { ...dist },
    deque: [],
    message: "0-1 BFS complete. All shortest paths finalized."
  });

  return steps;
}

export default function ZeroOneBFSSimulation() {
  ...
  const steps = useMemo(() => generate01Steps(graph.nodes, graph.edges), [graph]);
  const step = steps[Math.min(stepIndex, steps.length - 1)] || (steps.length ? steps[0] : { dist: {}, deque: [], message: "Add nodes to start." });
  ...
  const toggleWeight = useCallback((edgeId: string) => {
    setGraph(g => ({ ...g, edges: g.edges.map(e => e.id === edgeId ? { ...e, weight: e.weight === 0 ? 1 : 0 } : e) }));
  }, []);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          <div className="badge">Specialized BFS</div>
          <h1>0-1 BFS Algorithm</h1>
          <p className="description">
            Efficient shortest path algorithm for graphs where edge weights are only <strong>0</strong> or <strong>1</strong>. 
            By using a <strong>Double-Ended Queue (Deque)</strong>, it achieves linear complexity <code>O(V + E)</code>, bypassing the overhead of a Priority Queue.
          </p>
      ...
      <section className="guide">
        <div className="guide-content">
          <div className="card">
            ...
            <h3>Deque Strategy</h3>
            <p>Weight 0 edges are <code>unshifted</code> to the front, while weight 1 edges are <code>pushed</code> to the back. This maintains the sorted order naturally.</p>
          </div>
          <div className="card highlight">
            <div className="card-icon"><Zap size={20} /></div>
            <h3>Why 0-1?</h3>
            <p>If all weights are the same, BFS works. If they are 0/1, we can still avoid Dijkstra's sorting by prioritizing 0-weight "free" moves.</p>
          </div>
        </div>
      </section>
      ...
      <section id="simulator" className="simulator">
        <div className="editor">
          ...
          <select value={edgeWeight} onChange={(e) => setEdgeWeight(Number(e.target.value))}>
          ...
          <aside className="movable-panel">
            ...
            <div className="data-section">
              ...
              <div className="deque-viz">
                <div className="deque-end">Front</div>
                <div className="deque-body">
                  {step.deque.map((id, idx) => (
                    <div key={idx} className="deque-node">{id}</div>
                  ))}
                  {step.deque.length === 0 && <div className="empty">Empty</div>}
                </div>
                <div className="deque-end">Back</div>
              </div>
            </div>
              ...
              <div className="dist-table">
                {graph.nodes.map(n => (
                  <div key={n.id} className={`dist-row ${step.u === n.id ? 'highlight' : ''}`}>
                    <span className="node-id">Node {n.id}</span>
          ...
          <div className="canvas-area">
            <svg ref={svgRef} viewBox="0 0 600 500" onPointerDown={handleCanvasClick} onPointerMove={handleMove} onPointerUp={() => setDragging(null)}>
              ...
              {graph.edges.map(edge => {
                ...
                return (
                  <g key={edge.id} onPointerDown={(e) => { e.stopPropagation(); setSelected({ type: "edge", id: edge.id }); }} onContextMenu={(e) => handleContextMenu(e, { type: "edge", id: edge.id })} onDoubleClick={() => toggleWeight(edge.id)}>
                    <line x1={from.x} y1={from.y} x2={to.x} y2={to.y} 
                      className={`edge ${isActive ? 'active' : ''} weight-${edge.weight} ${isSelected ? 'selected' : ''}`} 
                    />
              ...
              {graph.nodes.map(node => {
                const isVisited = step.dist?.[node.id] !== undefined && step.dist[node.id] !== Infinity;
            ...
            <div className="gesture-hint-canvas">
               🖱️ Click to Add • ↔️ Drag to Move • ⌨️ Shift+Drag to Join • 🔘 Double-Click to Toggle Weight
            </div>
          </div>
        </div>
      </section>
      ...
      <style jsx>{`
        ...
        .deque-viz { display: flex; flex-direction: column; align-items: center; gap: 8px; padding: 16px; background: var(--bg); border-radius: 16px; border: 1px solid var(--border); }
        .deque-body { display: flex; gap: 6px; }
        .deque-end { font-size: 10px; font-weight: 700; color: var(--text-dim); text-transform: uppercase; }
        .deque-node { width: 32px; height: 32px; background: var(--panel-light); border: 1px solid var(--border); border-radius: 8px; display: flex; align-items: center; justify-content: center; font-weight: 800; font-family: monospace; font-size: 14px; color: var(--accent-light); }
        ...
        .edge.weight-0 { stroke: var(--green); opacity: 0.6; }
