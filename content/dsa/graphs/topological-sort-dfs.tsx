...
import { Play, Pause, RotateCcw, ChevronLeft, ChevronRight, Info, ListOrdered, MousePointer2 } from "lucide-react";

interface GraphNode { id: string; x: number; y: number; }
interface GraphEdge { id: string; from: string; to: string; }
interface GraphData { nodes: GraphNode[]; edges: GraphEdge[]; }

interface DFSTopoStep {
  type: string;
  visited: string[];
  recStack: string[];
  sorted: string[];
...
function generateDFSTopoSteps(nodes: GraphNode[], edges: GraphEdge[]): DFSTopoStep[] {
  ...
  function dfs(u: string): boolean {
    visited.add(u);
    recStack.add(u);
    steps.push({ type: "visit", u, visited: Array.from(visited), recStack: Array.from(recStack), sorted: [...sorted], message: `Visiting ${u}. Push to recursion stack.`, hasCycle: false });
    const neighbors = edges.filter(e => e.from === u).map(e => e.to);
    for (const v of neighbors) {
      if (!visited.has(v)) { if (dfs(v)) return true; }
      else if (recStack.has(v)) {
        steps.push({ type: "cycle", u, v, visited: Array.from(visited), recStack: Array.from(recStack), sorted: [...sorted], message: `Back-edge ${u}→${v}! Cycle detected.`, hasCycle: true });
        return true;
      }
    }
    recStack.delete(u);
    sorted.unshift(u);
    steps.push({ type: "finish", u, visited: Array.from(visited), recStack: Array.from(recStack), sorted: [...sorted], message: `Finished ${u}. Prepending to result.`, hasCycle: false });
    return false;
  }

  for (const node of nodes) if (!visited.has(node.id)) if (dfs(node.id)) break;
  if (!steps.some(s => s.hasCycle)) steps.push({ type: "done", visited: Array.from(visited), recStack: [], sorted: [...sorted], message: "Topological Sort Complete.", hasCycle: false });
  return steps;
}

export default function TopoDFSSimulation() {
  ...
  const [stackPos, setStackPos] = useState({ x: 480, y: 20 });
  const [activeDragPanel, setActiveDragPanel] = useState<string | null>(null);
  const [dragOffset, setDragOffset] = useState({ x: 0, y: 0 });

  const steps = useMemo(() => generateDFSTopoSteps(graph.nodes, graph.edges), [graph]);
    ...
    <main className="page" data-theme={theme}>
      <section className="hero">
        <div className="content-width">
          <span className="eyebrow">Graph Algorithms • Ordering</span>
          <h1>Topological Sort (DFS)</h1>
          <p className="description">
            A linear ordering of vertices in a <strong>Directed Acyclic Graph (DAG)</strong>. 
            The DFS approach works by visiting nodes and adding them to the result list in reverse order of their completion.
          </p>
      ...
      <section className="detailed-guide">
        <div className="guide-grid">
          <article className="guide-card">
            <h2>Post-Order</h2>
            <p>We perform DFS and wait for a node to "finish" all its neighbors before adding it to our topological sort.</p>
          </article>
          <article className="guide-card">
            <h2>The Stack</h2>
            <p>A recursion stack tracks the currently active nodes. This helps detect if the graph is truly acyclic.</p>
          </article>
          <article className="guide-card highlight">
            <h2>Cycle Check</h2>
            <p>If we revisit a node that is already on the recursion stack, we've found a cycle! Topological sort is impossible.</p>
          </article>
          <article className="guide-card">
            <h2>Reverse Prep</h2>
            <p>Because we add nodes as they finish, the result is in reverse topological order. We prepend them to get the final list.</p>
          </article>
        </div>
      </section>
          ...
          <div className="workspace">
            <aside className="movable-panel">
              ...
              <div className="status-panel">
                <h2>Current Step</h2>
                <p className={`status-msg ${step.hasCycle ? 'error' : ''}`}>{step.message}</p>
            ...
            <svg ref={svgRef} viewBox="0 0 680 480" onPointerDown={handleCanvasClick} onPointerMove={handleMove} onPointerUp={() => { setDragging(null); setActiveDragPanel(null); }}>
              ...
              {graph.edges.map(edge => {
                ...
                const cycle = step.hasCycle && step.u === edge.from && step.v === edge.to;
                ...
                return (
                  <g key={edge.id} onPointerDown={(e) => { e.stopPropagation(); setSelected({ type: "edge", id: edge.id }); }}>
                    <line x1={from.x} y1={from.y} x2={to.x} y2={to.y} className={`edge ${cycle ? 'cycle' : ''} ${isSelected ? 'selected' : ''}`} markerEnd="url(#arrow)" />
              ...
              {graph.nodes.map(node => {
                ...
                const finished = step.sorted.includes(node.id);
                const inStack = step.recStack.includes(node.id);
                const active = step.u === node.id;
                const isSelected = selected?.type === "node" && selected.id === node.id;
                return (
                  <g key={node.id} className={`node ${visited ? 'visited' : ''} ${finished ? 'finished' : ''} ${inStack ? 'on-stack' : ''} ${active ? 'active' : ''} ${isSelected ? 'selected' : ''}`} onPointerDown={(e) => handleNodePointerDown(e, node)} onContextMenu={(e) => handleContextMenu(e, { type: "node", id: node.id })}>
              ...
              <foreignObject x={orderPos.x} y={orderPos.y} width="160" height="250" className="movable-panel">
                <div className="panel-container" data-theme={theme}>
                  ...
                  <div className="panel-content order-viz">
                    {step.sorted.map((id, idx) => (
                      <div key={idx} className="order-item">{idx+1}. {id}</div>
                    ))}
                    {step.sorted.length === 0 && <span className="empty-msg">Waiting...</span>}
                  </div>
                </div>
              </foreignObject>
              ...
              <foreignObject x={stackPos.x} y={stackPos.y} width="160" height="250" className="movable-panel">
                <div className="panel-container" data-theme={theme}>
                  ...
                  <div className="panel-content stack-viz">
                    {step.recStack.map((id, idx) => (
                      <div key={idx} className="stack-item">{id}</div>
                    ))}
                    {step.recStack.length === 0 && <span className="empty-msg">Empty</span>}
                  </div>
                </div>
              </foreignObject>
            </svg>
      ...
      <style jsx>{`
        ...
        .edge { stroke: var(--border); stroke-width: 2.5; opacity: 0.5; transition: all 0.3s; pointer-events: none; }
        .edge.cycle { stroke: var(--red); stroke-width: 4; stroke-dasharray: 6 3; }
        ...
        .node.finished circle { stroke: var(--green); fill: color-mix(in srgb, var(--green) 10%, var(--panel)); }
        .node.on-stack circle { stroke: var(--amber); stroke-width: 5; }
        ...
        .order-item { padding: 4px 8px; background: color-mix(in srgb, var(--green) 10%, transparent); color: var(--green); border-radius: 6px; font-size: 11px; margin-bottom: 2px; font-weight: 700; }
        .stack-item { padding: 4px 8px; background: var(--panel2); color: var(--amber); border-radius: 6px; font-size: 11px; margin-bottom: 2px; font-weight: 700; text-align: center; }
